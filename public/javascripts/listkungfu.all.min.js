...
"shift+return",function(){e.toolbar.find("#list-item-new").effect("puff",{},300,function(){$(this).show();e._AddNewListItem(true)})});f.bind("keydown","l",function(){e.toolbar.find("#list-item-fullsize").effect("puff",{},300,function(){e.toolbar.find("#list-item-fullsize").trigger("click");$(this).show()});return false});ListKungFu&&ListKungFu.LayoutCenter&&f.draggable({helper:function(){return $('<div class="list-item-drag-helper">'+f.find(".list-item-content").text().substring(0,20)+" . . .</div>").get(0)},
...
function(){b._AddNewListItem(true)});b.toolbar.find("#showCompleted").bind("change",function(c){b._ToggleCompleted(c.target.checked);b.selectedListItem.element.focus()});b.toolbar.find("#list-item-completed").bind("click",function(){b._MarkCompleted(b.selectedListItem.element,b.selectedListItem.data)});b.toolbar.find("#list-item-delete").bind("click",function(){var c=function(){b._DeleteListItem(b.selectedListItem.element,b.selectedListItem.data)};if(typeof b.deleteDialog==="undefined")b.deleteDialog=
$.confirmationDialog("Delete List Item",c,"Cancel","Do you really want to delete?");b.deleteDialog.dialog("open")});b.toolbar.find("#list-item-edit").bind("click",function(){b.selectedListItem.element.trigger("dblclick")});b.toolbar.find("#list-item-fullsize").bind("click",function(){b._ToggleFullsize(b.selectedListItem.element)});b.header.append(b.toolbar)},_AddNewListItem:function(b){var c=this,i=c.element.data("data-list");if(c.listItemList.find("#new_list_item").length>0)return false;ListListItem.New({successCallback:function(e){if($.browser.msie&&
c.listItemList.find("#new_list_item").length>0)return false;var f=$(e);c.listItemList.prepend(f);c._SetupDeadlineButton(f);c._SetupCustomDeadlinePicker();f.find("textarea").markItUp(mySettings);b&&f.find("textarea").focus();c.listItemList.find(".row").hide();f.find("#cancel-edit").bind("click",function(j){j.preventDefault();f.hide("slow",function(){$(this).remove();c.listItemList.find(".row").show();c.listItemList.find(".row").first().focus()})});f.find(".deadline-button").bind("click",function(j){j.preventDefault();
var m=f.serializeForm();m.list_item.body=m.list_item.body.replace(/\n/g,"\n\n");m.list_item.deadlineType=$(j.target).attr("data-deadline");m.list_item.customDeadlineValue=$("#custom-deadline-value").val();ListListItem.Create({send:m,successCallback:function(s,u){f.hide("slow",function(){c._AddListItem(u,s,true);$(this).remove();c.toolbar.find("#list-item-search").trigger("ClearValue");c.listItemList.find(".row").show();c.listItemList.find(".row").first().focus();c.listItemList.find(".row").first().find(".handle").height(c.listItemList.find(".row").first().find(".list-item-content").height())})},
lists:i.list.id});return false});f.find("textarea").bind("keydown","esc",function(){f.find("#cancel-edit").trigger("click")})},lists:i.list.id})},_create:function(){var b=this;b.wrapper=$('<div class="ui-layout-content" id="list-item-wrapper"></div>');b.listItemList=$('<div id="list-item-list"></div>');b.header=$('<div class="list-item-header"></div>');b.element.append(b.header);b.element.append(b.wrapper);b.wrapper.append(b.listItemList);b.selectedText="";b._TriggerResize();$(document).bind("keydown",
