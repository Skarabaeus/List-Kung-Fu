...
b.listItemList.find("#new_list_item").length>0)return false;var e=$(i);b.listItemList.prepend(e);b._SetupDeadlineButton(e);b._SetupCustomDeadlinePicker();e.find("textarea.editorarea").tinymce(ListKungFu.TinyMCEDefaultOptions);b.listItemList.find(".row").hide();e.find("#cancel-edit").bind("click",function(f){f.preventDefault();e.hide("slow",function(){$(this).remove();b.listItemList.find(".row").show();b.listItemList.find(".row").first().focus()})});e.find(".deadline-button").bind("click",function(f){f.preventDefault();
var j=e.serializeForm();j.list_item.deadlineType=$(f.target).attr("data-deadline");j.list_item.customDeadlineValue=$("#custom-deadline-value").val();ListListItem.Create({send:j,successCallback:function(m,s){e.hide("slow",function(){b._AddListItem(s,m,true);$(this).remove();b.toolbar.find("#list-item-search").trigger("ClearValue");b.listItemList.find(".row").show();b.listItemList.find(".row").first().focus();b.listItemList.find(".row").first().find(".handle").height(b.listItemList.find(".row").first().find(".list-item-content").height())})},
