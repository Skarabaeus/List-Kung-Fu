...
TagSelected:function(b,c){ListKungFu.LayoutWest.ListView("FilterByTags",c)}});$("#dashboard").bind("click",function(){ListKungFu.LayoutCenter.ListItemView("RemoveList");ListKungFu.LayoutCenter.Dashboard("Show");return false});$("#dashboard").trigger("click")});
...
function(){var i=$(this).val();if(i==="")b.listlist.find(".row").show();else{b.listlist.find(".row:not(:Contains('"+i+"'))").hide();b.listlist.find(".row:Contains('"+i+"')").show()}b._AdjustHeight();b._triggerResize()});c.find("#list-search-cancel").bind("click",function(){c.find("#search-list").val("").trigger("keyup")});c.find("#search-list").bind("ClearValue",function(i){$(i.target).val("");b.listlist.find(".row").show()});c.find("#list-edit").bind("click",function(){b.listlist.hide("slide",{direction:"left"},
"slow",function(){if(b.listForm!=null){b.listForm.remove();b.listForm=null}b.listForm=$('<div class="ui-layout-content" id="list-form"></div>');b.wrapper.append(b.listForm);List.Edit({successCallback:function(i){b.listForm.append(i);b.listForm.show("slide",{direction:"left"},"slow",function(){b.listForm.find("#list_title").first().focus()});b.listForm.find("#list-back-button").bind("click",function(){b._HideForm()});b.listForm.find("#list_title").bind("keydown","esc",function(){b._HideForm()});b.listForm.bind("submit",
function(e){e.preventDefault();e=$(this).find("form").serializeForm();List.Update({send:e,successCallback:function(f,j,m,s,u){b._ClearFormErrors(b.listForm);u===false?b._HideForm(j,f):b._HighlightFormErrors(b.listForm,u)},lists:b.selectedList.data.list.id});return false})},lists:b.selectedList.data.list.id})})});return c},_GetListElement:function(b,c){var i=this,e=$($.mustache(c,b.list));e.attr("tabindex",0);e.data("data",b);e.addClass("row");$.browser.msie&&e.bind("click",function(){e.focus();return false});
e.bind("keydown","down",function(f){f.preventDefault();$(f.target).nextAll("div:visible").first().focus();return false});e.bind("keydown","up",function(f){f.preventDefault();$(f.target).prevAll("div:visible").first().focus();return false});e.bind("focus",function(f){f=$(f.target);i.selectedList={element:f,data:f.data("data")};i.listlist.find(".row").removeClass("selected-row");f.addClass("selected-row");i.listlist.find(".assigned-tags").hide();f.find(".assigned-tags").show();i._AdjustHeight()});e.bind("keydown dblclick",
...
$(this).focus()}):this.listlist.find(".row").first().effect("highlight",{},300,function(){$(this).show();$(this).focus()})},_RegisterGlobalKeyboardShortcuts:function(){var b=this;$(document).bind("keydown","ctrl+l",function(){b._SelectLastList()});$(document).bind("keydown","ctrl+f",function(){b.toolbar.find("#search-list").focus()});$(document).bind("keydown","shift+f",function(){b.toolbar.find("#search-list").focus();return false})},_ShowListView:function(b,c){var i=this;i.listlist.show("slide",
{direction:"left"},"slow",function(){if(b){var e=i._GetListElement(b,c);i._IsNewList(b)?i.listlist.prepend(e):i.selectedList.element.replaceWith(e);i._AdjustHeight();i.selectedList={data:b,element:e}}i.selectedList&&i.selectedList.element.focus()})},_HideForm:function(b,c){var i=this;i.listForm.hide("slide",{direction:"left"},"slow",function(){i._ShowListView(b,c);i.listForm.trigger("FormHidden");i.selectedList?i.selectedList.element.focus():i.listlist.find(".row").first().focus();i.listForm.remove();
i.listForm=null})},_FilterByTags:function(){var b=this;b.toolbar.find("#search-list").trigger("keyup");b.listlist.find(".row:visible").each(function(){var c=$(this),i=c.data("data"),e=0;if(b.selectedTags.length>0){for(var f=0;f<i.list.tags.length;f++)for(var j=0;j<b.selectedTags.length;j++)b.selectedTags[j]===i.list.tags[f].id&&e++;e===b.selectedTags.length?c.show():c.hide()}})},_create:function(){var b=this;b.selectedList=null;b.wrapper=b.element.find("div#list-wrapper");b.toolbar=b._CreateToolbar();
b.listForm=null;b.selectedTags=[];List.Index({successCallback:function(c,i){b.wrapper.find(".row").remove();b._AddListToDOM(i,c);b.listlist.find(".row").first().focus()}});b._RegisterGlobalKeyboardShortcuts()},options:{},SetupDroppable:function(b){var c=this;c.listlist.find(".row").each(function(){var i=$(this);i.droppable("destroy");switch(b){case "listitem":i.droppable(c._DroppableConfigListItems(i));break;case "tag":i.droppable(c._DroppableConfigTags())}})},ReloadLists:function(){var b=this;List.Index({successCallback:function(c,
i){b.wrapper.find(".row").remove();b._AddListToDOM(i,c);b.toolbar.find("#search-list").trigger("keyup");b._FilterByTags()}})},SelectList:function(){this._SelectLastList()},FilterByTags:function(b){this.selectedTags=b;this._FilterByTags()},destroy:function(){this.toolbar.remove();this.listForm.remove();this.wrapper.find(".row").remove()}})})();
