...
c.flash.message!=="")b.onFlashUpdate(c.flash.notice,c.flash.error,c.flash.warning,c.flash.message)};c.baseURL=b.base_url||document.location.protocol+"//"+document.location.hostname+(document.location.port!==""?":"+document.location.port:"")+"/";c.route=b.route;c.DefaultCallback=function(e,g,f,j){c.SetFlash(j);var m=g.data,x="";if(g.template&&$.trim(g.template)!=="")x=$("#"+g.template).html();g=g.errors;if(!$.isArray(m))for(var y in m){m[y].isSaved=m[y].id&&m[y].id!==null?true:false;m[y].isNew=m[y].id&&
m[y].id!==null?false:true}typeof e==="function"&&e(x,m,f,j,g);c.ClearFlash()};c.DefaultErrorCallback=function(){$('<div id="dialog-confirm"> \t\t\t<span class="ui-icon ui-icon-alert" style="float:left; margin:0 7px 20px 0;"> \t\t\t</span>Something went wrong :-( Either try again or reload the page!</div>').dialog({resizable:false,modal:true,buttons:{"Try again":function(){$(this).dialog("close")},Reload:function(){window.location.reload()}}})};c.ConstructRoute=function(e){for(var g="",f="",j=0;j<
c.route.length;j++){g+=c.route[j];f=e[c.route[j]]||"";if(f!=="")g+="/"+f;if(j!==c.route.length-1)g+="/"}return g};c.Index=function(e){var g=e.successCallback||null,f=c.ConstructRoute(e);jQuery.retrieveJSON(c.baseURL+f,e.send||{},function(j,m,x){c.DefaultCallback(g,j,m,x)}).error(c.DefaultErrorCallback)};c.Show=function(e){var g=e.successCallback||null;e=c.ConstructRoute(e);$.ajax({url:c.baseURL+e,dataType:"json",type:"GET",processData:false,contentType:"application/json",beforeSend:function(f){f.setRequestHeader("X-CSRF-Token",
...
j,m){c.DefaultErrorCallback(f,j,m)}})};c.Create=function(e){var g=e.successCallback||null,f=c.ConstructRoute(e);e=JSON.stringify(e.send);$.ajax({url:c.baseURL+f,dataType:"json",type:"POST",processData:false,contentType:"application/json",data:e,beforeSend:function(j){j.setRequestHeader("X-CSRF-Token",$('meta[name="csrf-token"]').attr("content"))},success:function(j,m,x){c.DefaultCallback(g,j,m,x)},error:function(j,m,x){c.DefaultErrorCallback(j,m,x)}})};c.Edit=function(e){var g=e.successCallback||
...
type:"POST",processData:false,contentType:"application/json",data:e,beforeSend:function(j){j.setRequestHeader("X-Http-Method-Override","PUT");j.setRequestHeader("X-CSRF-Token",$('meta[name="csrf-token"]').attr("content"))},success:function(j,m,x){c.DefaultCallback(g,j,m,x)},error:function(j,m,x){c.DefaultErrorCallback(j,m,x)}})};c.Destroy=function(e){var g=e.successCallback||null;e=c.ConstructRoute(e);$.ajax({url:c.baseURL+e,dataType:"json",type:"POST",processData:false,contentType:"application/json",
...
f.is("form")?f.serializeArray():[];b.ajax({url:m,data:y,dataType:x,type:j.toUpperCase(),beforeSend:function(p){f.trigger("ajax:loading",p)},success:function(p,B,w){f.trigger("ajax:success",[p,B,w])},complete:function(p){f.trigger("ajax:complete",p)},error:function(p,B,w){f.trigger("ajax:failure",[p,B,w])}})}f.trigger("ajax:after")}}});b("body").delegate("a[data-confirm], button[data-confirm], input[data-confirm]","click.rails",function(){var f=b(this);if(f.triggerAndReturn("confirm"))if(!confirm(f.attr("data-confirm")))return false});
...
_TriggerResize:function(){this._trigger("ContentDimensionsChanged",0,{})},_NothingToDoText:function(){return'<p class="nothing-todo">no items scheduled</p>'},_TriggerReinitOfPanes:function(){this._trigger("ReinitPanes",0,{})},_SetupCustomDeadlinePicker:function(b){var c=this,e=$(this),g=b.find(".dashboard-list-item-deadline-hidden"),f=b.data("data"),j=f.list_item.deadline_date.split("-"),m=j[0],x=j[1],y=j[2];g.datepicker({minDate:new Date,dateFormat:"yy,mm,dd",beforeShow:function(p,B){B.input.datepicker("setDate",
m+","+x+","+y)},onSelect:function(p){f.list_item.deadlineType="customdeadline";f.list_item.customDeadlineValue=p;f.list_item.template="dashboard";ListListItem.Update({successCallback:function(B,w){var F=b.parent();b.hide("slow",function(){b.remove();F.find(".dashboard-item").length===0&&F.append(c._NothingToDoText())});c._CreateDashboardItem(w,B,{highlightCategory:true,addType:"prepend"})},lists:f.list_item.list_id,list_items:f.list_item.id,send:f});e.datepicker("hide")}})},_AppendToDashboard:function(b,
...
function(c){b._ToggleCompleted(c.target.checked);b.selectedListItem&&b.selectedListItem.element.focus()});b.toolbar.find("#list-item-completed").bind("click",function(){b.selectedListItem&&b._MarkCompleted(b.selectedListItem.element,b.selectedListItem.data)});b.toolbar.find("#list-item-delete").bind("click",function(){var c=function(){b._DeleteListItem(b.selectedListItem.element,b.selectedListItem.data)};if(typeof b.deleteDialog==="undefined")b.deleteDialog=$.confirmationDialog("Delete List Item",
c,"Cancel","Do you really want to delete?");b.deleteDialog.dialog("open")});b.toolbar.find("#list-item-edit").bind("click",function(){b.selectedListItem&&b.selectedListItem.element.trigger("edit")});b.header.append(b.toolbar)},_AddNewListItem:function(){var b=this,c=b.element.data("data-list");if(b.listItemList.find("#new_list_item").length>0)return false;ListListItem.New({successCallback:function(e,g){if($.browser.msie&&b.listItemList.find("#new_list_item").length>0)return false;var f=$($.mustache(e,
g.list_item));b.listItemList.prepend(f);b._SetupDeadlineButton(f);b._SetupCustomDeadlinePicker();var j=ListKungFu.TinyMCEDefaultOptions;j.height=b.listItemList.parent("#list-item-wrapper").height()-150;f.find("textarea.editorarea").tinymce(j);b.listItemList.find(".row").hide();f.find("#cancel-edit").bind("click",function(m){m.preventDefault();f.hide("slow",function(){$(this).remove();b.listItemList.find(".row").show();b.listItemList.find(".row").first().focus();$.browser.msie&&b.listItemList.find(".row").first().click()})});
f.find(".deadline-button").bind("click",function(m){m.preventDefault();var x=f.serializeForm();x.list_item.deadlineType=$(m.target).attr("data-deadline");x.list_item.customDeadlineValue=$("#custom-deadline-value").val();ListListItem.Create({send:x,successCallback:function(y,p){f.hide("slow",function(){b._AddListItem(p,y,true);$(this).remove();b.toolbar.find("#list-item-search").trigger("ClearValue");b.listItemList.find(".row").show();b.listItemList.find(".row").first().focus();b.listItemList.find(".row").first().find(".handle").height(b.listItemList.find(".row").first().find(".list-item-content").height())})},
...
this.listForm.remove();this.wrapper.find(".row").remove()}})})();(function(b){b.fn.serializeForm=function(){var c=b(this).find('*[name*="]"]'),e={};b.each(c,function(g,f){var j=b(f),m=j.get(0).name.split("[")[0],x=j.get(0).name.split("[")[1];x=x.replace("]","");j=j.val();e[m]=e[m]||{};e[m][x]=j});return e}})(jQuery);
...
(function(){function b(p){return p<10?"0"+p:p}function c(p){f.lastIndex=0;return f.test(p)?'"'+p.replace(f,function(B){var w=x[B];return typeof w==="string"?w:"\\u"+("0000"+B.charCodeAt(0).toString(16)).slice(-4)})+'"':'"'+p+'"'}function e(p,B){var w,F,I,H,z=j,G,C=B[p];if(C&&typeof C==="object"&&typeof C.toJSON==="function")C=C.toJSON(p);if(typeof y==="function")C=y.call(B,p,C);switch(typeof C){case "string":return c(C);case "number":return isFinite(C)?String(C):"null";case "boolean":case "null":return String(C);
case "object":if(!C)return"null";j+=m;G=[];if(Object.prototype.toString.apply(C)==="[object Array]"){H=C.length;for(w=0;w<H;w+=1)G[w]=e(w,C)||"null";I=G.length===0?"[]":j?"[\n"+j+G.join(",\n"+j)+"\n"+z+"]":"["+G.join(",")+"]";j=z;return I}if(y&&typeof y==="object"){H=y.length;for(w=0;w<H;w+=1){F=y[w];if(typeof F==="string")if(I=e(F,C))G.push(c(F)+(j?": ":":")+I)}}else for(F in C)if(Object.hasOwnProperty.call(C,F))if(I=e(F,C))G.push(c(F)+(j?": ":":")+I);I=G.length===0?"{}":j?"{\n"+j+G.join(",\n"+j)+
"\n"+z+"}":"{"+G.join(",")+"}";j=z;return I}}if(typeof Date.prototype.toJSON!=="function"){Date.prototype.toJSON=function(){return isFinite(this.valueOf())?this.getUTCFullYear()+"-"+b(this.getUTCMonth()+1)+"-"+b(this.getUTCDate())+"T"+b(this.getUTCHours())+":"+b(this.getUTCMinutes())+":"+b(this.getUTCSeconds())+"Z":null};String.prototype.toJSON=Number.prototype.toJSON=Boolean.prototype.toJSON=function(){return this.valueOf()}}var g=/[\u0000\u00ad\u0600-\u0604\u070f\u17b4\u17b5\u200c-\u200f\u2028-\u202f\u2060-\u206f\ufeff\ufff0-\uffff]/g,
f=/[\\\"\x00-\x1f\x7f-\x9f\u00ad\u0600-\u0604\u070f\u17b4\u17b5\u200c-\u200f\u2028-\u202f\u2060-\u206f\ufeff\ufff0-\uffff]/g,j,m,x={"\u0008":"\\b","\t":"\\t","\n":"\\n","\u000c":"\\f","\r":"\\r",'"':'\\"',"\\":"\\\\"},y;if(typeof JSON.stringify!=="function")JSON.stringify=function(p,B,w){var F;m=j="";if(typeof w==="number")for(F=0;F<w;F+=1)m+=" ";else if(typeof w==="string")m=w;if((y=B)&&typeof B!=="function"&&(typeof B!=="object"||typeof B.length!=="number"))throw Error("JSON.stringify");return e("",
{"":p})};if(typeof JSON.parse!=="function")JSON.parse=function(p,B){function w(I,H){var z,G,C=I[H];if(C&&typeof C==="object")for(z in C)if(Object.hasOwnProperty.call(C,z)){G=w(C,z);if(G!==undefined)C[z]=G;else delete C[z]}return B.call(I,H,C)}var F;p=String(p);g.lastIndex=0;if(g.test(p))p=p.replace(g,function(I){return"\\u"+("0000"+I.charCodeAt(0).toString(16)).slice(-4)});if(/^[\],:{}\s]*$/.test(p.replace(/\\(?:["\\\/bfnrt]|u[0-9a-fA-F]{4})/g,"@").replace(/"[^"\\\n\r]*"|true|false|null|-?\d+(?:\.\d*)?(?:[eE][+\-]?\d+)?/g,
"]").replace(/(?:^|:|,)(?:\s*\[)+/g,""))){F=eval("("+p+")");return typeof B==="function"?w({"":F},""):F}throw new SyntaxError("JSON.parse");}})();
...
resizerCursor:"e-resize"},west:{paneSelector:".ui-layout-west",size:200,resizerCursor:"w-resize"},center:{paneSelector:".ui-layout-center"}},B={slide:{all:{duration:"fast"},north:{direction:"up"},south:{direction:"down"},east:{direction:"right"},west:{direction:"left"}},drop:{all:{duration:"slow"},north:{direction:"up"},south:{direction:"down"},east:{direction:"right"},west:{direction:"left"}},scale:{all:{duration:"fast"}}},w={id:Math.floor(Math.random()*1E4),container:{},north:{},south:{},east:{},
west:{},center:{}},F={north:"south",south:"north",east:"west",west:"east"},I=function(d){if(typeof d=="string")return true;else if(typeof d=="object")try{return d.constructor.toString().match(/string/i)!==null}catch(h){}return false},H=function(d){return typeof d=="string"||I(d)?b.trim(d):d},z=function(d,h){return Math.max(d,h)},G=function(d){var h={defaults:{fxSettings:{}},north:{fxSettings:{}},south:{fxSettings:{}},east:{fxSettings:{}},west:{fxSettings:{}},center:{fxSettings:{}}};d=d||{};if(d.effects||
d.defaults||d.north||d.south||d.west||d.east||d.center)h=b.extend(h,d);else b.each(d,function(i,k){a=i.split("__");h[a[1]?a[0]:"defaults"][a[1]?a[1]:a[0]]=k});return h},C=function(d,h,i){function k(s){n=t[s];if(n.doCallback){cpPane=n.callback.split(",")[1];cpPane!=s&&cpPane!=h&&k(cpPane,true)}else{n.doCallback=true;n.callback=l}}var l=d+","+h+","+(i?1:0),n;b.each(t.borderPanes.split(","),function(s,q){if(t[q].isMoving){k(q);return false}})},ca=function(d){d=t[d];t.isLayoutBusy=false;delete d.isMoving;
if(d.doCallback&&d.callback){d.doCallback=false;var h=d.callback.split(","),i=h[2]>0?true:false;if(h[0]=="open")V(h[1],i);else h[0]=="close"&&T(h[1],i);if(!d.doCallback)d.callback=null}},L=function(d,h){if(h){var i;try{if(typeof h=="function")i=h;else if(typeof h!="string")return;else if(h.indexOf(",")>0){var k=h.split(",");i=eval(k[0]);if(typeof i=="function"&&k.length>1)return i(k[1])}else i=eval(h);if(typeof i=="function")return i(d,E[d],b.extend({},w[d]),b.extend({},p[d]),p.name)}catch(l){}}},
M=function(d,h){var i=0,k=false,l="";if(!b.browser.msie)if(b.curCSS(d[0],"display",true)=="none"){k=true;l=b.curCSS(d[0],"visibility",true);d.css({display:"block",visibility:"hidden"})}i=parseInt(b.curCSS(d[0],h,true),10)||0;if(k){d.css({display:"none"});l&&l!="hidden"&&d.css({visibility:l})}return i},R=function(d,h){var i;if(I(d)){d=H(d);i=E[d]}else i=b(d);if(h<=0)return 0;else h>0||(h=I(d)?S(d):i.outerWidth());return b.boxModel?h-M(i,"paddingLeft")-M(i,"paddingRight")-(b.curCSS(i[0],"borderLeftStyle",
true)=="none"?0:M(i,"borderLeftWidth"))-(b.curCSS(i[0],"borderRightStyle",true)=="none"?0:M(i,"borderRightWidth")):h},O=function(d,h){var i;if(I(d)){d=H(d);i=E[d]}else i=b(d);if(h<=0)return 0;else h>0||(h=I(d)?S(d):i.outerHeight());return b.boxModel?h-M(i,"paddingTop")-M(i,"paddingBottom")-(b.curCSS(i[0],"borderTopStyle",true)=="none"?0:M(i,"borderTopWidth"))-(b.curCSS(i[0],"borderBottomStyle",true)=="none"?0:M(i,"borderBottomWidth")):h},S=function(d,h){var i=E[d],k=p[d],l=w[d],n=h?k.spacing_open:
0;k=h?k.spacing_closed:0;return!i||l.isHidden?0:l.isClosed||l.isSliding&&h?k:t[d].dir=="horz"?i.outerHeight()+n:i.outerWidth()+n},da=function(d){var h=A,i=t[d].dir,k=w[d],l=E[F[d]],n=p[d].spacing_open,s=p[F[d]].spacing_open,q=!l?0:i=="horz"?l.outerHeight():l.outerWidth();l=k.minSize||0;i=Math.min(k.maxSize||9999,(i=="horz"?h.innerHeight:h.innerWidth)-n-q-s);var u,r;switch(d){case "north":u=h.offsetTop+l;r=h.offsetTop+i;break;case "west":u=h.offsetLeft+l;r=h.offsetLeft+i;break;case "south":u=h.offsetTop+
h.innerHeight-i;r=h.offsetTop+h.innerHeight-l;break;case "east":u=h.offsetLeft+h.innerWidth-i;r=h.offsetLeft+h.innerWidth-l}b.extend(k,{minSize:l,maxSize:i,minPosition:u,maxPosition:r})},ma=function(){var d={top:S("north",true),bottom:S("south",true),left:S("west",true),right:S("east",true),width:0,height:0};with(d){width=A.innerWidth-left-right;height=A.innerHeight-bottom-top;top+=A.top;bottom+=A.bottom;left+=A.left;right+=A.right}return d},na=function(d){var h={},i,k,l;b.each("Left,Right,Top,Bottom".split(","),
function(){i=H(this);k=h["border"+i]=M(d,"border"+i+"Width");l=h["padding"+i]=M(d,"padding"+i);h["offset"+i]=k+l;if(d==N)h[i.toLowerCase()]=b.boxModel?l:0});h.innerWidth=h.outerWidth=d.outerWidth();h.innerHeight=h.outerHeight=d.outerHeight();if(b.boxModel){h.innerWidth-=h.offsetLeft+h.offsetRight;h.innerHeight-=h.offsetTop+h.offsetBottom}return h},ra=function(d,h,i,k){var l=window.layout=window.layout||{};l=l.timers=l.timers||{};d="layout_"+w.id+"_"+d+"_"+h;l[d]||(l[d]=setTimeout(i,k))},ea=function(d,
h){var i=window.layout=window.layout||{};i=i.timers=i.timers||{};var k="layout_"+w.id+"_"+d+"_"+h;if(i[k]){clearTimeout(i[k]);delete i[k];return true}else return false},sa=function(){try{if(N[0].tagName=="BODY"){b("html").css({height:"100%",overflow:"hidden"});b("body").css({position:"relative",height:"100%",overflow:"hidden",margin:0,padding:0,border:"none"})}else{var d={overflow:"hidden"},h=N.css("position"),i=N.css("height");if(!N.hasClass("ui-layout-pane")){if(!h||"fixed,absolute,relative".indexOf(h)<
0)d.position="relative";if(!i||i=="auto")d.height="100%"}N.css(d)}}catch(k){}A=w.container=na(N)},ta=function(){b.each(t.borderPanes.split(","),function(d,h){var i=p[h];if(i.enableCursorHotkey||i.customHotkey){b(document).keydown(e);return false}})},ua=function(){c=G(c);if(c.effects){b.extend(B,c.effects);delete c.effects}b.each("name,scrollToBookmarkOnLoad".split(","),function(i,k){if(c[k]!==undefined)p[k]=c[k];else if(c.defaults[k]!==undefined){p[k]=c.defaults[k];delete c.defaults[k]}});b.each("paneSelector,resizerCursor,customHotkey".split(","),
...
y.paneClass;if(!o.resizerClass)o.resizerClass=y.resizerClass;if(!o.togglerClass)o.togglerClass=y.togglerClass;b.each(["_open","_close",""],function(l,n){var s="fxName"+n,q="fxSpeed"+n,u="fxSettings"+n;o[s]=c[k][s]||c[k].fxName||c.defaults[s]||c.defaults.fxName||o[s]||o.fxName||h[s]||h.fxName||"none";var r=o[s];if(r=="none"||!b.effects||!b.effects[r]||!B[r]&&!o[u]&&!o.fxSettings)r=o[s]="none";r=B[r]||{};s=r.all||{};r=r[k]||{};o[u]=b.extend({},s,r,h.fxSettings||{},h[u]||{},o.fxSettings,o[u],c.defaults.fxSettings,
c.defaults[u]||{},c[k].fxSettings,c[k][u]||{});o[q]=c[k][q]||c[k].fxSpeed||c.defaults[q]||c.defaults.fxSpeed||o[q]||o[u].duration||o.fxSpeed||o.fxSettings.duration||h.fxSpeed||h.fxSettings.duration||r.duration||s.duration||"normal"})})},oa=function(){b.each(t.allPanes.split(","),function(){var d=H(this),h=p[d],i=w[d],k=t[d].dir,l=h.size=="auto"||isNaN(h.size)?0:h.size,n=h.minSize||1,s=h.maxSize||9999,q=h.spacing_open||0,u=h.paneSelector,r={},v;X[d]=false;if(u.substr(0,1)==="#")v=E[d]=N.find(u+":first");
else{v=E[d]=N.children(u+":first");v.length||(v=E[d]=N.children("form:first").children(u+":first"))}if(!v.length){E[d]=false;return true}v.attr("pane",d).addClass(h.paneClass+" "+h.paneClass+"-"+d);if(d!="center"){i.isClosed=false;i.isSliding=false;i.isResizing=false;i.isHidden=false;i.noRoom=false;t[d].pins=[]}r=b.extend({visibility:"visible",display:"block"},t.defaults.cssReq,t[d].cssReq);h.applyDefaultStyles&&b.extend(r,t.defaults.cssDef,t[d].cssDef);v.css(r);r={};switch(d){case "north":r.top=
A.top;r.left=A.left;r.right=A.right;break;case "south":r.bottom=A.bottom;r.left=A.left;r.right=A.right;break;case "west":r.left=A.left;break;case "east":r.right=A.right}if(k=="horz"){if(l===0||l=="auto"){v.css({height:"auto"});l=v.outerHeight()}l=z(l,n);l=Math.min(l,s);l=Math.min(l,A.innerHeight-q);r.height=z(1,O(d,l));i.size=l;i.maxSize=s;i.minSize=z(n,l-r.height+1);v.css(r)}else if(k=="vert"){if(l===0||l=="auto"){v.css({width:"auto","float":"left"});l=v.outerWidth();v.css({"float":"none"})}l=z(l,
n);l=Math.min(l,s);l=Math.min(l,A.innerWidth-q);r.width=z(1,R(d,l));i.size=l;i.maxSize=s;i.minSize=z(n,l-r.width+1);v.css(r);W(d,null,true)}else if(d=="center"){v.css(r);W("center",null,true)}if(h.initClosed&&h.closable){v.hide().addClass("closed");i.isClosed=true}else if(h.initHidden||h.initClosed){ha(d,true);i.isHidden=true}else v.addClass("open");h.showOverflowOnHover&&v.hover(g,f);if(h.contentSelector){i=X[d]=v.children(h.contentSelector+":first");if(!i.length){X[d]=false;return true}i.css(t.content.cssReq);
h.applyDefaultStyles&&i.css(t.content.cssDef);v.css({overflow:"hidden"})}})},va=function(){b.each(t.borderPanes.split(","),function(){var d=H(this),h=p[d],i=w[d],k=h.resizerClass,l=h.togglerClass,n=E[d];P[d]=false;Y[d]=false;if(!(!n||!h.closable&&!h.resizable)){var s=t[d].edge;n=n.is(":visible");var q="-"+d,u=n?"-open":"-closed",r;r=P[d]=b("<span></span>");n&&h.resizable||!n&&h.slidable&&r.attr("title",h.sliderTip).css("cursor",h.sliderCursor);r.attr("id",h.paneSelector.substr(0,1)=="#"?h.paneSelector.substr(1)+
"-resizer":"").attr("resizer",d).css(t.resizers.cssReq).css(s,A[s]+S(d)).addClass(k+" "+k+q+" "+k+u+" "+k+q+u).appendTo(N);h.applyDefaultStyles&&r.css(t.resizers.cssDef);if(h.closable){k=Y[d]=b("<div></div>");k.attr("id",h.paneSelector.substr(0,1)=="#"?h.paneSelector.substr(1)+"-toggler":"").css(t.togglers.cssReq).attr("title",n?h.togglerTip_open:h.togglerTip_closed).click(function(v){ba(d);v.stopPropagation()}).mouseover(function(v){v.stopPropagation()}).addClass(l+" "+l+q+" "+l+u+" "+l+q+u).appendTo(r);
if(h.togglerContent_open)b("<span>"+h.togglerContent_open+"</span>").addClass("content content-open").css("display",i.isClosed?"none":"block").appendTo(k);if(h.togglerContent_closed)b("<span>"+h.togglerContent_closed+"</span>").addClass("content content-closed").css("display",i.isClosed?"block":"none").appendTo(k);h.applyDefaultStyles&&k.css(t.togglers.cssDef);n||ia(d,true)}}});Z("all",true)},wa=function(){var d=typeof b.fn.draggable=="function",h;b.each(t.borderPanes.split(","),function(){var i=
H(this),k=p[i],l=w[i];if(!d||!E[i]||!k.resizable){k.resizable=false;return true}var n=k.resizerClass,s=n+"-drag",q=n+"-"+i+"-drag",u=n+"-dragging",r=n+"-"+i+"-dragging",v=false,D=P[i];l.isClosed||D.attr("title",k.resizerTip).css("cursor",k.resizerCursor);D.draggable({containment:N[0],axis:t[i].dir=="horz"?"y":"x",delay:200,distance:1,helper:"clone",opacity:k.resizerDragOpacity,zIndex:t.zIndex.resizing,start:function(){if(false===L(i,k.onresize_start))return false;l.isResizing=true;ea(i,"closeSlider");
...
" "+r).children().css("visibility","hidden");v=true;l.isSliding&&E[i].css("zIndex",t.zIndex.sliding)}if(K.position[h]<l.minPosition)K.position[h]=l.minPosition;else if(K.position[h]>l.maxPosition)K.position[h]=l.maxPosition},stop:function(J,K){var Q=K.position,U;D.removeClass(s+" "+q);switch(i){case "north":U=Q.top;break;case "west":U=Q.left;break;case "south":U=A.outerHeight-Q.top-D.outerHeight();break;case "east":U=A.outerWidth-Q.left-D.outerWidth()}ja(i,U-A[t[i].edge]);b("div.ui-layout-mask").remove();
l.isResizing=false}})})},ha=function(d,h){var i=p[d],k=w[d],l=E[d],n=P[d];if(!(!l||k.isHidden))if(false!==L(d,i.onhide_start)){k.isSliding=false;n&&n.hide();if(h||k.isClosed){k.isClosed=true;k.isHidden=true;l.hide();W(t[d].dir=="horz"?"all":"center");L(d,i.onhide_end||i.onhide)}else{k.isHiding=true;T(d,false)}}},fa=function(d,h){var i=p[d],k=w[d],l=P[d];if(E[d]&&k.isHidden)if(false!==L(d,i.onshow_start)){k.isSliding=false;k.isShowing=true;l&&i.spacing_open>0&&l.show();h===false?T(d,true):V(d)}},ba=
function(d){var h=w[d];if(h.isHidden)fa(d);else h.isClosed?V(d):T(d)},T=function(d,h,i){function k(){ia(d,true);K||L(d,q.onclose_end||q.onclose);if(K)L(d,q.onshow_end||q.onshow);if(Q)L(d,q.onhide_end||q.onhide);ca(d)}var l=E[d],n=P[d],s=Y[d],q=p[d],u=w[d];i=!i&&!u.isClosed&&q.fxName_close!="none";var r=t[d].edge,v=q.resizerClass,D=q.togglerClass,J="-"+d,K=u.isShowing,Q=u.isHiding;delete u.isShowing;delete u.isHiding;if(!(!l||!q.resizable&&!q.closable))if(!(!h&&u.isClosed&&!K))if(t.isLayoutBusy)C("close",
d,h);else if(!(!K&&false===L(d,q.onclose_start))){t[d].isMoving=true;t.isLayoutBusy=true;u.isClosed=true;if(Q)u.isHidden=true;else if(K)u.isHidden=false;m(d,false);u.isSliding||W(t[d].dir=="horz"?"all":"center");if(n){n.css(r,A[r]).removeClass(v+"-open "+v+J+"-open").removeClass(v+"-sliding "+v+J+"-sliding").addClass(v+"-closed "+v+J+"-closed");q.resizable&&n.draggable("disable").css("cursor","default").attr("title","");s&&s.removeClass(D+"-open "+D+J+"-open").addClass(D+"-closed "+D+J+"-closed").attr("title",
q.togglerTip_closed);Z()}if(i){ga(d,true);l.hide(q.fxName_close,q.fxSettings_close,q.fxSpeed_close,function(){ga(d,false);u.isClosed&&k()})}else{l.hide();k()}}},V=function(d,h,i){function k(){u.isSliding||W(t[d].dir=="vert"?"center":"all");if(n){n.css(r,A[r]+S(d)).removeClass(v+Q+" "+v+J+Q).addClass(v+K+" "+v+J+K).addClass(!u.isSliding?"":v+U+" "+v+J+U);q.resizable?n.draggable("enable").css("cursor",q.resizerCursor).attr("title",q.resizerTip):n.css("cursor","default");s&&s.removeClass(D+Q+" "+D+J+
Q).addClass(D+K+" "+D+J+K).attr("title",q.togglerTip_open);Z("all")}aa(d);m(d,!u.isSliding);L(d,q.onopen_end||q.onopen);if(ka)L(d,q.onshow_end||q.onshow);ca(d)}var l=E[d],n=P[d],s=Y[d],q=p[d],u=w[d];i=!i&&u.isClosed&&q.fxName_open!="none";var r=t[d].edge,v=q.resizerClass,D=q.togglerClass,J="-"+d,K="-open",Q="-closed",U="-sliding",ka=u.isShowing;delete u.isShowing;if(!(!l||!q.resizable&&!q.closable))if(u.isClosed||u.isSliding)if(u.isHidden&&!ka)fa(d,true);else if(t.isLayoutBusy)C("open",d,h);else if(false!==
L(d,q.onopen_start)){t[d].isMoving=true;t.isLayoutBusy=true;u.isSliding&&!h&&la(d,false);u.isClosed=false;if(ka)u.isHidden=false;da(d);u.size>u.maxSize&&l.css(t[d].sizeType,z(1,t[d].dir=="horz"?O(d,u.maxSize):R(d,u.maxSize)));ia(d,false);if(i){ga(d,true);l.show(q.fxName_open,q.fxSettings_open,q.fxSpeed_open,function(){ga(d,false);u.isClosed||k()})}else{l.show();k()}}},ga=function(d,h){var i=E[d];if(h){i.css({zIndex:t.zIndex.animation});if(d=="south")i.css({top:A.top+A.innerHeight-i.outerHeight()});
else d=="east"&&i.css({left:A.left+A.innerWidth-i.outerWidth()})}else{w[d].isSliding||i.css({zIndex:t.zIndex.pane_normal});if(d=="south")i.css({top:"auto"});else d=="east"&&i.css({left:"auto"})}},ia=function(d,h){var i=p[d],k=P[d],l=i.slideTrigger_open;if(k&&i.slidable){if(l!="click"&&l!="dblclick"&&l!="mouseover")l="click";k[h?"bind":"unbind"](l,xa).css("cursor",h?i.sliderCursor:"default").attr("title",h?i.sliderTip:"")}},la=function(d,h){function i(s){ea(d,"closeSlider");s.stopPropagation()}var k=
p[d].slideTrigger_close,l=E[d],n=P[d];w[d].isSliding=h;ea(d,"closeSlider");l.css({zIndex:h?t.zIndex.sliding:t.zIndex.pane_normal});n.css({zIndex:h?t.zIndex.sliding:t.zIndex.resizer_normal});if(k!="click"&&k!="mouseout")k="mouseout";if(h){l.bind(k,pa);n.bind(k,pa);k="mouseout";l.bind("mouseover",i);n.bind("mouseover",i)}else{l.unbind(k);n.unbind(k);k="mouseout";l.unbind("mouseover");n.unbind("mouseover");ea(d,"closeSlider")}},xa=function(){var d=b(this).attr("resizer");if(w[d].isClosed){la(d,true);
V(d,true)}},pa=function(){function d(){la(i,false);k.isClosed||T(i)}var h=b(this),i=h.attr("pane")||h.attr("resizer");h=p[i];var k=w[i];k.isClosed||k.isResizing||(h.slideTrigger_close=="click"?d():ra(i,"closeSlider",d,300))},ja=function(d,h){var i=t[d].edge,k=t[d].dir,l=p[d],n=w[d],s=E[d],q=P[d];da(d);n.minSize=z(n.minSize,l.minSize);if(l.maxSize>0)n.maxSize=Math.min(n.maxSize,l.maxSize);h=z(h,n.minSize);h=Math.min(h,n.maxSize);n.size=h;q.css(i,h+A[i]);s.css(t[d].sizeType,z(1,t[d].dir=="horz"?O(d,
h):R(d,h)));n.isSliding||W(k=="horz"?"all":"center");Z();aa(d);L(d,l.onresize_end||l.onresize)},W=function(d,h,i){if(!d||d=="all")d="east,west,center";var k=ma();h&&b.extend(k,h);b.each(d.split(","),function(){if(E[this]){var l=H(this),n=p[l],s=w[l],q=E[l],u=P[l],r=true,v={};if(l=="center"){k=ma();v=b.extend({},k);v.width=z(1,R(l,v.width));v.height=z(1,O(l,v.height));r=v.width>1&&v.height>1;if(b.browser.msie&&(!b.boxModel||b.browser.version<7)){E.north&&E.north.css({width:R(E.north,A.innerWidth)});
E.south&&E.south.css({width:R(E.south,A.innerWidth)})}}else{v.top=k.top;v.bottom=k.bottom;v.height=z(1,O(l,k.height));r=v.height>1}if(r){q.css(v);if(s.noRoom){s.noRoom=false;if(s.isHidden)return;else fa(l,!s.isClosed)}if(!i){aa(l);L(l,n.onresize_end||n.onresize)}}else if(!s.noRoom){s.noRoom=true;if(!s.isHidden)if(i){q.hide();u&&u.hide()}else ha(l)}}})},aa=function(d){if(!d||d=="all")d=t.allPanes;b.each(d.split(","),function(){if(X[this]){var h=H(this),i=p[h].contentIgnoreSelector,k=E[h];h=X[h];var l=
h[0],n=O(k);k.children().each(function(){if(this!=l){var s=b(this);if(!i||!s.is(i))n-=s.outerHeight()}});if(n>0)n=O(h,n);n<1?h.hide():h.css({height:n}).show()}})},Z=function(d,h){if(!d||d=="all")d=t.borderPanes;b.each(d.split(","),function(){var i=H(this),k=p[i],l=w[i],n=E[i],s=P[i],q=Y[i];if(!(!n||!s||!k.resizable&&!k.closable)){var u=t[i].dir,r=l.isClosed?"_closed":"_open";i=k["spacing"+r];var v=k["togglerAlign"+r];r=k["togglerLength"+r];var D;if(i==0)s.hide();else{!l.noRoom&&!l.isHidden&&s.show();
if(u=="horz"){D=n.outerWidth();s.css({width:z(1,R(s,D)),height:z(1,O(s,i)),left:M(n,"left")})}else{D=n.outerHeight();s.css({height:z(1,O(s,D)),width:z(1,R(s,i)),top:A.top+S("north",true)})}if(q){if(r==0||l.isSliding&&k.hideTogglerOnSlide){q.hide();return}else q.show();if(!(r>0)||r=="100%"||r>D){r=D;n=0}else if(typeof v=="string")switch(v){case "top":case "left":n=0;break;case "bottom":case "right":n=D-r;break;default:n=Math.floor((D-r)/2)}else{n=parseInt(v);n=v>=0?n:D-r+n}D=k.togglerContent_open?
q.children(".content-open"):false;var J=k.togglerContent_closed?q.children(".content-closed"):false;v=l.isClosed?J:D;if(D)D.css("display",l.isClosed?"none":"block");if(J)J.css("display",l.isClosed?"block":"none");if(u=="horz"){l=R(q,r);q.css({width:z(0,l),height:z(1,O(q,i)),left:n});v&&v.css("marginLeft",Math.floor((l-v.outerWidth())/2))}else{l=O(q,r);q.css({height:z(0,l),width:z(1,R(q,i)),top:n});v&&v.css("marginTop",Math.floor((l-v.outerHeight())/2))}}if(h&&k.initHidden){s.hide();q&&q.hide()}}}})},
qa=function(){var d=A.innerWidth,h=A.innerHeight;A=w.container=na(N);var i=A.innerHeight<h,k=A.innerWidth<d,l,n;if(i||k)b.each(["south","north","east","west"],function(s,q){l=w[q];n=t[q].dir;if(!l.isClosed&&(i&&n=="horz"||k&&n=="vert")){da(q);l.size>l.maxSize&&ja(q,l.maxSize)}});W("all");Z("all")},N=b(this).css({overflow:"hidden"}),E={},X={},P={},Y={},t={allPanes:"north,south,east,west,center",borderPanes:"north,south,east,west",zIndex:{resizer_normal:1,pane_normal:2,mask:4,sliding:100,resizing:1E4,
...
l.stopPropagation()});x(i,h,!k.isClosed&&!k.isSliding);t[h].pins.push(d)}},allowOverflow:g,resetOverflow:f,cssWidth:R,cssHeight:O}}})(jQuery);
...
(function(b){function c(f,j,m){if(jQuery.isFunction(j)){m=j;j=null}var x=f+"?"+b.param(j||{});if(g[x])return false;g[x]=true;return jQuery.ajax({type:"GET",url:f,data:j,success:function(y,p){delete g[x];if(undefined===y)window.navigator.onLine||(e=function(){c(f,j,m)});else m(y,p)},error:function(){delete g[x]},dataType:"json",ifModified:true})}var e=null,g={};if(typeof b.support.localStorage==="undefined")b.support.localStorage=!!window.localStorage;if(b.support.localStorage){b(window).bind("online",
function(){e&&e()});b(window).bind("offline",function(){jQuery.event.trigger("ajaxStop")});b.retrieveJSON=function(f,j,m){function x(){for(var H=[],z=0;z<localStorage.length;++z){var G=localStorage.key(z);/:date$/.test(G)&&H.push(G)}H.sort(function(C,ca){var L=localStorage[C],M=localStorage[ca];return L<M?-1:L>M?+1:0});for(z=0;z<H.length/2;++z){G=H[z];delete localStorage[G];delete localStorage[G.substr(0,G.length-5)]}}function y(){return c(f,j,function(H,z){if(z=="notmodified")return false;for(;;)try{localStorage[w]=
JSON.stringify(H);localStorage[w+":date"]=new Date;break}catch(G){if(G.name=="QUOTA_EXCEEDED_ERR"||G.name=="NS_ERROR_DOM_QUOTA_REACHED")x()}m(H,z,F&&{cachedAt:I,retrievedAt:p})})}if(b.isFunction(j)){m=j;j={}}var p=new Date,B=b.param(j),w="offline.jquery:"+f+":"+B,F=localStorage[w],I=new Date(Date.parse(localStorage[w+":date"]));if(F)if(m(b.parseJSON(F),"cached",{cachedAt:I})===false)return false;if(window.navigator.onLine)B=y();else{e=y;B=true}return B};b.clearJSON=function(f,j){var m=b.param(j||
