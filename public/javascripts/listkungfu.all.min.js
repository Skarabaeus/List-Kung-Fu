var ListKungFu={LayoutWest:$("#mainlayout-west"),LayoutNorth:$("#mainlayout-north"),LayoutEast:$("#mainlayout-east"),LayoutSouth:$("#mainlayout-south"),LayoutCenter:$("#mainlayout-center")};
...
theme_advanced_buttons3:"tablecontrols,|,hr,removeformat,visualaid,|,sub,sup,|,cite,abbr,acronym|,advhr,|,print,|,fullscreen",theme_advanced_toolbar_location:"top",theme_advanced_toolbar_align:"left",theme_advanced_statusbar_location:"bottom",theme_advanced_resizing:false,width:"100%"};jQuery.expr[":"].Contains=function(b,c,i){return jQuery(b).text().toLowerCase().indexOf(i[3].toLowerCase())>=0};jQuery.expr[":"].HasExactValue=function(b,c,i){return jQuery.trim(jQuery(b).text().toLowerCase())===i[3].toLowerCase()};
...
e.selectedListItem.element.find(".list-item-info").hide();m.hide();e.selectedListItem.element.prepend(m);j=ListKungFu.TinyMCEDefaultOptions;j.height=e.element.find("#list-item-wrapper").height()-110;m.find("textarea.editorarea").tinymce(j);e._SetupDeadlineButton(m);e._SetupCustomDeadlinePicker();s===false&&e._ToggleFullsize(f);e.selectedListItem.element.find(".handle").hide();m.show("slow",function(){m.find("textarea").focus()});m.find("textarea").bind("keydown","esc",function(){m.find("#cancel-edit").trigger("click")});
m.find(".deadline-button").bind("keydown click","return",function(u){u.preventDefault();var p=f.find("form").serializeForm();p.list_item.deadlineType=$(u.target).attr("data-deadline");p.list_item.customDeadlineValue=$("#custom-deadline-value").val();ListListItem.Update({send:p,successCallback:function(E,y){m.hide("slow",function(){$(this).remove();e.selectedListItem.element.find(".handle").show();f.find(".list-item-content").html(y.list_item.body).show();f.find(".list-item-info").show();f.find(".list-item-deadline").html(y.list_item.deadline_in_words);
