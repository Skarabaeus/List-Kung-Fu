...
theme_advanced_buttons3:"tablecontrols,|,hr,removeformat,visualaid,|,sub,sup,|,cite,abbr,acronym|,advhr,|,print,|,fullscreen",theme_advanced_toolbar_location:"top",theme_advanced_toolbar_align:"left",theme_advanced_statusbar_location:"bottom",theme_advanced_resizing:false,init_instance_callback:ListKungFu.TinyMceInitAutoFocus,width:"100%",setup:function(b){var c=false;b.onKeyUp.add(function(i,e){if(e.keyCode===18)return c=false});b.onKeyDown.add(function(i,e){if(e.keyCode===27){ListKungFu.LayoutCenter.ListItemView("CloseEditor");
return false}if(e.keyCode===18){c=true;return false}if(c===true&&e.keyCode===83){e.preventDefault();ListKungFu.LayoutCenter.ListItemView("FocusElement","saveButton");return false}})}};jQuery.expr[":"].Contains=function(b,c,i){return jQuery(b).text().toLowerCase().indexOf(i[3].toLowerCase())>=0};jQuery.expr[":"].HasExactValue=function(b,c,i){return jQuery.trim(jQuery(b).text().toLowerCase())===i[3].toLowerCase()};
...
b.listItemList.find("#new_list_item").length>0)return false;var e=$(i);b.listItemList.prepend(e);b._SetupDeadlineButton(e);b._SetupCustomDeadlinePicker();i=ListKungFu.TinyMCEDefaultOptions;i.height=b.listItemList.parent("#list-item-wrapper").height()-150;e.find("textarea.editorarea").tinymce(i);b.listItemList.find(".row").hide();e.find("#cancel-edit").bind("click",function(f){f.preventDefault();e.hide("slow",function(){$(this).remove();b.listItemList.find(".row").show();b.listItemList.find(".row").first().focus();
