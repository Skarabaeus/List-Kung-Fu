...
e.selectedListItem.element.find(".list-item-info").hide();e.selectedListItem.element.removeClass("selected-row");m.hide();e.selectedListItem.element.prepend(m);j=ListKungFu.TinyMCEDefaultOptions;j.height=e.element.find("#list-item-wrapper").height()-110;m.find("textarea.editorarea").tinymce(j);e._SetupDeadlineButton(m);e._SetupCustomDeadlinePicker();s===false&&e._ToggleFullsize(f);e.selectedListItem.element.find(".handle").hide();m.show("slow",function(){m.find("textarea").focus()});m.find("textarea").bind("keydown",
"esc",function(){m.find("#cancel-edit").trigger("click")});m.find(".deadline-button").bind("keydown click","return",function(u){u.preventDefault();var p=f.find("form").serializeForm();p.list_item.deadlineType=$(u.target).attr("data-deadline");p.list_item.customDeadlineValue=$("#custom-deadline-value").val();ListListItem.Update({send:p,successCallback:function(E,y){m.hide("slow",function(){$(this).remove();e.selectedListItem.element.find(".handle").show();f.find(".list-item-content").html(y.list_item.body).show();
f.find(".list-item-info").show();f.find(".list-item-deadline").html(y.list_item.deadline_in_words);f.find(".handle").height(f.find(".list-item-content").height());s===false&&e._ToggleFullsize(f);e._SetSelectedListItem(f,y);f.data("data",y);e._Filter();f.focus()})},lists:e.selectedListItem.data.list_item.list_id,list_items:e.selectedListItem.data.list_item.id});return false});m.find("#cancel-edit").bind("keydown click","return",function(u){u.preventDefault();e.selectedListItem.element.find(".handle").show();
m.hide("slow",function(){$(this).remove();e.selectedListItem.element.find(".list-item-content").show();e.selectedListItem.element.find(".list-item-info").show();s===false&&e._ToggleFullsize(f);e._Filter();f.focus()});return false});return false},lists:e.selectedListItem.data.list_item.list_id,list_items:e.selectedListItem.data.list_item.id})});f.bind("keydown","left",function(){e._trigger("SelectLastList",0,{})});f.bind("keydown","shift+return",function(){e.toolbar.find("#list-item-new").effect("puff",
{},300,function(){$(this).show();e._AddNewListItem()})});f.bind("keydown","l",function(){e.toolbar.find("#list-item-fullsize").effect("puff",{},300,function(){e.toolbar.find("#list-item-fullsize").trigger("click");$(this).show()});return false});ListKungFu&&ListKungFu.LayoutCenter&&f.draggable({helper:function(){return $('<div class="list-item-drag-helper">'+f.find(".list-item-content").text().substring(0,20)+" . . .</div>").get(0)},appendTo:ListKungFu.LayoutCenter,revert:"invalid",handle:".handle",
cursorAt:{top:10,left:10},iframeFix:true,start:function(){document.onselectstart=function(){return false};e._trigger("StartedDraggingListItem",0,{dragType:"listitem"})},stop:function(){document.onselectstart=null}});e._CorrectHeight(f)},_ToggleFullsize:function(b){if(b.data("isFullsize")===true){this._CorrectHeight(b,false);b.data("isFullsize",false);this.toolbar.find("#list-item-fullsize").button("option","icons",{primary:"ui-icon-zoomin"})}else{this._CorrectHeight(b,true);b.data("isFullsize",true);
this.toolbar.find("#list-item-fullsize").button("option","icons",{primary:"ui-icon-zoomout"})}},_CorrectHeight:function(b,c){b.height()>150&&!c?b.height(150):b.height("auto");this._TriggerResize()},_DeleteListItem:function(b,c){ListListItem.Destroy({successCallback:function(){b.hide("slow",function(){var i=b.next(".row");i.length>0||(i=b.prev(".row"));i.focus();b.remove()})},lists:c.list_item.list_id,list_items:c.list_item.id})},_MarkCompleted:function(b,c){var i=this;c.list_item.completed=true;ListListItem.Update({successCallback:function(){var e=
...
$('<div id="list-item-toolbar"><div id="list-title"></div><div id="list-item-toolbar-buttons"><button id="list-item-completed">Completed [space]</button><button id="list-item-new">Create [shift+return]</button><button id="list-item-delete">Delete [del]</button><button id="list-item-edit">Edit [return]</button><button id="list-item-fullsize">Fullsize [l]</button><input type="checkbox" id="showCompleted"/><label for="showCompleted">Show Completed Items</label></div></div>');b.listName=b.toolbar.find("#list-title");
b.toolbar.find("#list-item-new").button({text:false,icons:{primary:"ui-icon-plusthick"}});b.toolbar.find("#list-item-edit").button({text:false,icons:{primary:"ui-icon-pencil"}});b.toolbar.find("#list-item-completed").button({text:false,icons:{primary:"ui-icon-check"}});b.toolbar.find("#list-item-delete").button({text:false,icons:{primary:"ui-icon-trash"}});b.toolbar.find("#list-item-fullsize").button({text:false,icons:{primary:"ui-icon-zoomin"}});b.toolbar.find("#list-item-new").bind("click",function(){b._AddNewListItem()});
...
c,"Cancel","Do you really want to delete?");b.deleteDialog.dialog("open")});b.toolbar.find("#list-item-edit").bind("click",function(){b.selectedListItem.element.trigger("dblclick")});b.toolbar.find("#list-item-fullsize").bind("click",function(){b._ToggleFullsize(b.selectedListItem.element)});b.header.append(b.toolbar)},_AddNewListItem:function(){var b=this,c=b.element.data("data-list");if(b.listItemList.find("#new_list_item").length>0)return false;ListListItem.New({successCallback:function(i){if($.browser.msie&&
