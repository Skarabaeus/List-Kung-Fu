...
TagSelected:function(b,c){ListKungFu.LayoutWest.ListView("Filter",c)}});$("#dashboard").bind("click",function(){ListKungFu.LayoutCenter.ListItemView("RemoveList");ListKungFu.LayoutCenter.Dashboard("Show");return false});$("#dashboard").trigger("click")});
...
(function(b){b.confirmationDialog=function(c,i,e,f){dialog=b('<div id="dialog-confirm"> \t\t\t<span class="ui-icon ui-icon-alert" style="float:left; margin:0 7px 20px 0;"> \t\t\t</span>'+f+"</div>").dialog({resizable:false,modal:true,autoOpen:false,width:400,open:function(){dialog.parent().find(".ui-dialog-buttonpane > button").first().focus()}});f={};f[c]=function(){b(dialog).dialog("close");i()};f[e]=function(){b(dialog).dialog("close")};dialog.dialog("option","buttons",f);dialog.parent().find(".ui-dialog-buttonpane > button").bind("keydown",
"right",function(j){j=b(j.target);if(j.next("button").length>0){j.blur();j.next("button").focus()}return false});dialog.parent().find(".ui-dialog-buttonpane > button").bind("keydown","left",function(j){j=b(j.target);if(j.prev("button").length>0){j.blur();j.prev("button").focus()}return false});return dialog}})(jQuery);
...
f,i);break;case "this week":e._AppendToDashboard(e.thisweek,f,i);break;case "next week":e._AppendToDashboard(e.nextweek,f,i);break;default:e._AppendToDashboard(e.later,f,i)}f.data("data",b);f.find(".dashboard-list-item-title").bind("click",function(){e._trigger("OpenList",0,{selectedList:b.list_item})});f.find(".dashboard-item-more").bind("click",function(){e._trigger("OpenList",0,{selectedList:b.list_item})});f.find(".dashboard-item-completed").bind("click",function(){$that=$(this);$that.find(".confirmation-completed").remove();
var j=$('<div class="confirmation-completed">Completed? \t\t\t\t\t<span class="yes">Yes</span> / <span class="no">NO</span></div>');$that.append(j);$that.animate({width:"100%",paddingLeft:"14px"},200);j.find(".yes").bind("click",function(){b.list_item.completed=true;ListListItem.Update({successCallback:function(){f.stop(true,true).hide("slow",function(){f.remove()})},lists:b.list_item.list_id,list_items:b.list_item.id,send:b});j.find(".no").trigger("click");return false});j.find(".no").bind("click",
function(){$that.animate({width:"12px",paddingLeft:"0"},200,function(){j.remove()});return false})});f.find(".dashboard-item-more").bind("click",function(){e._trigger("OpenList",0,{selectedList:b.list_item})});e._SetupCustomDeadlinePicker(f);f.find(".dashboard-list-item-deadline").bind("click",function(){f.find(".dashboard-list-item-deadline").parent().find(".dashboard-list-item-deadline-hidden").datepicker("show")});return f},_create:function(){},options:{},Show:function(){var b=this;b.Hide();b.header=
$('<div id="dashboard-header" class="header"><h1>Dashboard</h1></div>');b.toolbar=$('<div id="dashboard-toolbar"><input type="text" id="dashboard-search" value=""/></div>');b.wrapper=$('<div class="ui-layout-content" id="dashboard-view"></div>');b.today=$('<div id="today" class="schedule-column"><h1>Today</h1><div data-type="scheduleColumn"></div></div>');b.tomorrow=$('<div id="tomorrow" class="schedule-column"><h1>Tomorrow</h1><div data-type="scheduleColumn"></div></div>');b.thisweek=$('<div id="thisweek" class="schedule-column"><h1>This Week</h1><div data-type="scheduleColumn"></div></div>');
b.nextweek=$('<div id="nextweek" class="schedule-column"><h1>Next Week</h1><div data-type="scheduleColumn"></div></div>');b.later=$('<div id="later"><h1>Later</h1><div data-type="scheduleColumn"></div></div>');b.element.append(b.header);b.header.append(b.toolbar);b.element.append(b.wrapper);b.wrapper.append(b.today);b.wrapper.append(b.tomorrow);b.wrapper.append(b.thisweek);b.wrapper.append(b.nextweek);b.nextweek.append(b.later);b.wrapper.append('<div style="clear:both;">&nbsp;</div>');b.toolbar.append('<div id="dashboard-search-cancel">&nbsp;</div><div style="clear:both;">&nbsp;</div>');
ListItem.Index({successCallback:function(c,i){$.each(i,function(e,f){b._CreateDashboardItem(f,c)});$('div[data-type="scheduleColumn"]').each(function(){$(this).children(".dashboard-item").length===0&&$(this).append(b._NothingToDoText())});b._TriggerReinitOfPanes();b._TriggerResize()},send:{show:"dashboard"}});b.toolbar.find("#dashboard-search").bind("keyup",function(){var c=$(this).val();if(c==="")b.wrapper.find(".dashboard-item").show();else{b.wrapper.find(".dashboard-item:not(:Contains('"+c+"'))").hide();
b.wrapper.find(".dashboard-item:Contains('"+c+"')").show()}});b.toolbar.find("#dashboard-search-cancel").bind("click",function(){b.toolbar.find("#dashboard-search").val("").trigger("keyup")})},Hide:function(){this.element.children().remove()},destroy:function(){}})})();
...
(function(){$.widget("ui.ListItemView",{_SetSelectedListItem:function(b,c){this.selectedListItem={element:b,data:c}},_SetupDeadlineButton:function(b){var c=b.find("#deadline-settings-wrapper"),i=b.find("#save-deadline");i.bind("click keydown","return",function(){i.data("settings-visible")?c.hide("fast",function(){c.find(".deadline-button").first();i.data("settings-visible",false)}):c.show("fast",function(){c.find(".deadline-button").first().focus();i.data("settings-visible",true)});return false})},
_SetupCustomDeadlinePicker:function(){$("#custom-deadline-display").datepicker({minDate:new Date,altField:"#custom-deadline-value",altFormat:"yy,mm,dd",dateFormat:"dd.mm.yy"});$("#custom-deadline-value").val($.datepicker.formatDate("yy,mm,dd",new Date));$("#custom-deadline-display").val($.datepicker.formatDate("dd.mm.yy",new Date))},_TriggerReinitOfPanes:function(){this._trigger("ReinitPanes",0,{})},_TriggerResize:function(){this._trigger("ContentDimensionsChanged",0,{})},_AddListItem:function(b,
c,i){var e=this,f=$($.mustache(c,b.list_item));f.data("data",b);f.data("isFullsize",false);i?e.listItemList.prepend(f):e.listItemList.append(f);f.find(".handle").height(f.height());f.bind("keydown","space",function(){e.toolbar.find("#list-item-completed").effect("puff",{},300,function(){$(this).show();f.find(".undo").length===0?e.toolbar.find("#list-item-completed").trigger("click"):f.find(".undo").trigger("click")})});f.bind("keydown","del",function(){e.toolbar.find("#list-item-delete").effect("puff",
{},300,function(){$(this).show();e.toolbar.find("#list-item-delete").trigger("click")})});f.bind("focus",function(j){if($(j.target).hasClass("row")){e.listItemList.find(".row").removeClass("selected-row");$(j.target).addClass("selected-row");$(j.target).data("data");if(e.toolbar)$(j.target).data("isFullsize")===true?e.toolbar.find("#list-item-fullsize").button("option","icons",{primary:"ui-icon-zoomout"}):e.toolbar.find("#list-item-fullsize").button("option","icons",{primary:"ui-icon-zoomin"});e._SetSelectedListItem($(j.target),
$(j.target).data("data"))}});$.browser.msie&&f.bind("click ",function(){f.focus();return false});f.bind("keydown","down",function(j){j.preventDefault();$(j.target).nextAll("div:visible").first().focus();return false});f.bind("keydown","up",function(j){j.preventDefault();$(j.target).prevAll("div:visible").first().focus();return false});f.bind("keydown dblclick","return",function(){if(f.find("form").length>0)return false;e.toolbar.find("#list-item-edit").effect("puff",{},300,function(){$(this).show()});
e.listItemList.find(".row").find("form").hide("slow",function(){$(this).remove()});ListListItem.Edit({successCallback:function(j){var m=$(j),s=f.data("isFullsize");e.listItemList.find(".row").hide();e.selectedListItem.element.show();m.hide();e.selectedListItem.element.prepend(m);m.find("textarea").markItUp(mySettings);e._SetupDeadlineButton(m);e._SetupCustomDeadlinePicker();s===false&&e._ToggleFullsize(f);e.selectedListItem.element.find(".handle").hide();m.show("slow",function(){m.find("textarea").focus()});
m.find("textarea").bind("keydown","esc",function(){m.find("#cancel-edit").trigger("click")});m.find(".deadline-button").bind("keydown click","return",function(u){u.preventDefault();var p=f.find("form").serializeForm();p.list_item.deadlineType=$(u.target).attr("data-deadline");p.list_item.customDeadlineValue=$("#custom-deadline-value").val();ListListItem.Update({send:p,successCallback:function(E,y){m.hide("slow",function(){$(this).remove();e.selectedListItem.element.find(".handle").show();f.find(".list-item-content").html(y.list_item.body_rendered);
f.find(".list-item-deadline").html(y.list_item.deadline_in_words);f.find(".handle").height(f.find(".list-item-content").height());s===false&&e._ToggleFullsize(f);e._SetSelectedListItem(f,y);f.data("data",y);e.listItemList.find(".row").show();f.focus()})},lists:e.selectedListItem.data.list_item.list_id,list_items:e.selectedListItem.data.list_item.id});return false});m.find("#cancel-edit").bind("keydown click","return",function(u){u.preventDefault();e.selectedListItem.element.find(".handle").show();
m.hide("slow",function(){$(this).remove();s===false&&e._ToggleFullsize(f);e.listItemList.find(".row").show();f.focus()});return false});return false},lists:e.selectedListItem.data.list_item.list_id,list_items:e.selectedListItem.data.list_item.id})});f.bind("keydown","left",function(){e._trigger("SelectLastList",0,{})});f.bind("keydown","shift+return",function(){e.toolbar.find("#list-item-new").effect("puff",{},300,function(){$(this).show();e._AddNewListItem()})});f.bind("keydown","l",function(){e.toolbar.find("#list-item-fullsize").effect("puff",
{},300,function(){e.toolbar.find("#list-item-fullsize").trigger("click");$(this).show()});return false});ListKungFu&&ListKungFu.LayoutCenter&&f.draggable({helper:function(){return $('<div class="list-item-drag-helper">'+f.find(".list-item-content").text().substring(0,20)+" . . .</div>").get(0)},appendTo:ListKungFu.LayoutCenter,revert:"invalid",handle:".handle",cursorAt:{top:10,left:10},iframeFix:true,start:function(){document.onselectstart=function(){return false};e._trigger("StartedDraggingListItem",
0,{dragType:"listitem"})},stop:function(){document.onselectstart=null}});e._CorrectHeight(f)},_ToggleFullsize:function(b){if(b.data("isFullsize")===true){this._CorrectHeight(b,false);b.data("isFullsize",false);this.toolbar.find("#list-item-fullsize").button("option","icons",{primary:"ui-icon-zoomin"})}else{this._CorrectHeight(b,true);b.data("isFullsize",true);this.toolbar.find("#list-item-fullsize").button("option","icons",{primary:"ui-icon-zoomout"})}},_CorrectHeight:function(b,c){b.height()>150&&
!c?b.height(150):b.height("auto");this._TriggerResize()},_DeleteListItem:function(b,c){var i=this;ListListItem.Destroy({successCallback:function(){b.hide("slow",function(){var e=b.next(".row");e.length>0||(e=b.prev(".row"));e.focus();b.remove();i.toolbar.find("#list-item-search").trigger("ClearValue")})},lists:c.list_item.list_id,list_items:c.list_item.id})},_MarkCompleted:function(b,c){var i=this;c.list_item.completed=true;ListListItem.Update({successCallback:function(){var e="list-item-undo-"+c.list_item.id;
b.find(".list-item-wrapper").hide("slow",function(){var f=$('<div class="undo clickable"><img src="/images/undo-icon.gif"/>Undo</div>');f.bind("click",function(){i._MarkUncompleted(b,c,e)});b.append(f);i._CorrectHeight(b);f.delay(5E3,e).queue(e,function(){b.hide("slow",function(){b.remove()})}).dequeue(e);i.toolbar&&i.toolbar.find("#showCompleted").get(0).checked===true&&i._ToggleCompleted(true);b.focus()})},lists:c.list_item.list_id,list_items:c.list_item.id,send:c})},_MarkUncompleted:function(b,
c,i){var e=this;i!==""&&b.clearQueue(i);c.list_item.completed=false;ListListItem.Update({successCallback:function(){b.find(".undo").hide("slow",function(){$(this).remove()});b.find(".list-item-wrapper").show("slow",function(){e._CorrectHeight(b)});e.toolbar&&e.toolbar.find("#showCompleted").get(0).checked===true&&e._ToggleCompleted(true)},lists:c.list_item.list_id,list_items:c.list_item.id,send:c})},_ToggleCompleted:function(b){var c=this;if(b===true){if(c.completedList){c.completedList.remove();
...
b.toolbar=$('<div id="list-item-toolbar"><div id="list-name"></div><div id="list-item-toolbar-buttons"><button id="list-item-completed">Completed [space]</button><button id="list-item-new">Create [shift+return]</button><button id="list-item-delete">Delete [del]</button><button id="list-item-edit">Edit [return]</button><button id="list-item-fullsize">Fullsize [l]</button><input type="input" id="list-item-search"/><input type="checkbox" id="showCompleted"/><label for="showCompleted">Show Completed Items</label><div id="list-item-search-cancel">&nbsp;</div></div></div>');
...
$.confirmationDialog("Delete List Item",c,"Cancel","Do you really want to delete?");b.deleteDialog.dialog("open")});b.toolbar.find("#list-item-edit").bind("click",function(){b.selectedListItem.element.trigger("dblclick")});b.toolbar.find("#list-item-fullsize").bind("click",function(){b._ToggleFullsize(b.selectedListItem.element)});b.toolbar.find("#list-item-search").bind("keyup",function(){var c=$(this).val();if(c==="")b.listItemList.find(".row").show();else{b.listItemList.find(".row:not(:Contains('"+
c+"'))").hide();b.listItemList.find(".row:Contains('"+c+"')").show()}});b.toolbar.find("#list-item-search-cancel").bind("click",function(){b.toolbar.find("#list-item-search").val("").trigger("keyup")});b.toolbar.find("#list-item-search").bind("ClearValue",function(c){$(c.target).val("");b.listItemList.find(".row").show()});b.header.append(b.toolbar)},_AddNewListItem:function(){var b=this,c=b.element.data("data-list");ListListItem.New({successCallback:function(i){var e=$(i);b.listItemList.prepend(e);
b._SetupDeadlineButton(e);b._SetupCustomDeadlinePicker();e.find("textarea").markItUp(mySettings);e.find("textarea").focus();b.listItemList.find(".row").hide();e.find("#cancel-edit").bind("click",function(f){f.preventDefault();e.hide("slow",function(){$(this).remove();b.listItemList.find(".row").show();b.listItemList.find(".row").first().focus()})});e.find(".deadline-button").bind("click",function(f){f.preventDefault();var j=e.serializeForm();j.list_item.body=j.list_item.body.replace(/\n/g,"\n\n");
...
s.append("<li>"+j[p]+"</li>")}u.parent(".field").append(m)}}},_ClearFormErrors:function(b){b.find(".field").removeClass("field_with_errors");b.find(".error_explanation").remove()},_DeleteList:function(b){var c=this,i=c.selectedList.element.next(),e=c.selectedList.element.prev();b=b||0;List.Destroy({successCallback:function(){c.selectedList.element.delay(b).fadeOut(function(){$(this).remove();i.length>0?i.focus():e.focus()});c._trigger("CloseList",0,{})},lists:c.selectedList.data.list.id})},_CreateToolbar:function(){var b=
this,c=$('<div id="list-toolbar"><button id="list-new">Create [shift+return]</button><button id="list-delete">Delete [del]</button><button id="list-edit">Edit [space]</button><input type="text" id="search-list" /><div id="list-search-cancel">&nbsp;</div></div>');b.element.find("div.header").append(c);c.find("#list-new").button({text:false,icons:{primary:"ui-icon-plusthick"}});c.find("#list-delete").button({text:false,icons:{primary:"ui-icon-trash"}});c.find("#list-edit").button({text:false,icons:{primary:"ui-icon-pencil"}});
c.find("#list-delete").bind("click",function(i){if(b.selectedList!==null){if(typeof b.deleteDialog==="undefined")b.deleteDialog=$.confirmationDialog("Delete List",function(){if(b.listForm!==null){b.listForm.bind("FormHidden",function(){b._DeleteList(1E3)});b._HideForm()}else b._DeleteList()},"Cancel","Do you really want to delete?");b.deleteDialog.dialog("open")}else $("#notice").text("Select the list which you want to delete").fadeIn().delay(5E3).fadeOut();$(i.target).blur();return false});c.find("#list-new").bind("click",
function(i){b.listlist.hide("slide",{direction:"left"},"slow",function(){if(b.listForm!=null){b.listForm.remove();b.listForm=null}b.listForm=$('<div class="ui-layout-content" id="list-form"></div>');b.wrapper.append(b.listForm);List.New({successCallback:function(e,f){b.listForm.append(e);b.listForm.show("slide",{direction:"left"},"slow",function(){b.listForm.find("#list_title").first().focus()});b._trigger("CloseList",0,{});b.listForm.find("#list-back-button").bind("click",function(){b._HideForm()});
b.listForm.find("#list_title").bind("keydown","esc",function(){b._HideForm()});b.listForm.bind("submit",function(j){j.preventDefault();j=$(this).find("form").serializeForm();f.list=j.list;List.Create({send:f,successCallback:function(m,s,u,p,E){b._ClearFormErrors(b.listForm);if(E===false){b._HideForm(s,m);b.toolbar.find("#search-list").trigger("ClearValue")}else b._HighlightFormErrors(b.listForm,E)}});return false});b.selectedList=null}})});$(i.target).blur();return false});c.find("#search-list").bind("keyup",
function(){b._Filter()});c.find("#list-search-cancel").bind("click",function(){c.find("#search-list").val("").trigger("keyup")});c.find("#search-list").bind("ClearValue",function(i){$(i.target).val("");b.listlist.find(".row").show()});c.find("#list-edit").bind("click",function(){b.listlist.hide("slide",{direction:"left"},"slow",function(){if(b.listForm!=null){b.listForm.remove();b.listForm=null}b.listForm=$('<div class="ui-layout-content" id="list-form"></div>');b.wrapper.append(b.listForm);List.Edit({successCallback:function(i){b.listForm.append(i);
b.listForm.show("slide",{direction:"left"},"slow",function(){b.listForm.find("#list_title").first().focus()});b.listForm.find("#list-back-button").bind("click",function(){b._HideForm()});b.listForm.find("#list_title").bind("keydown","esc",function(){b._HideForm()});b.listForm.bind("submit",function(e){e.preventDefault();e=$(this).find("form").serializeForm();List.Update({send:e,successCallback:function(f,j,m,s,u){b._ClearFormErrors(b.listForm);u===false?b._HideForm(j,f):b._HighlightFormErrors(b.listForm,
u)},lists:b.selectedList.data.list.id});return false})},lists:b.selectedList.data.list.id})})});return c},_GetListElement:function(b,c){var i=this,e=$($.mustache(c,b.list));e.attr("tabindex",0);e.data("data",b);e.addClass("row");$.browser.msie&&e.bind("click",function(){e.focus();return false});e.bind("keydown","down",function(f){f.preventDefault();$(f.target).nextAll("div:visible").first().focus();return false});e.bind("keydown","up",function(f){f.preventDefault();$(f.target).prevAll("div:visible").first().focus();
return false});e.bind("focus",function(f){f=$(f.target);i.selectedList={element:f,data:f.data("data")};i.listlist.find(".row").removeClass("selected-row");f.addClass("selected-row");i.listlist.find(".assigned-tags").hide();f.find(".assigned-tags").show();i._AdjustHeight()});e.bind("keydown dblclick","return",function(){i._trigger("OpenList",0,{selectedList:e.data("data")})});e.bind("keydown","right",function(){i._trigger("SelectListItem",0,{})});e.bind("keydown","shift+return",function(){i.toolbar.find("#list-new").effect("puff",
{},300,function(){$(this).show();$(this).trigger("click")});return false});e.bind("keydown","del",function(){i.toolbar.find("#list-delete").effect("puff",{},300,function(){$(this).show()}).trigger("click")});e.bind("keydown","space",function(){i.toolbar.find("#list-edit").effect("puff",{},300,function(){$(this).show();$(this).trigger("click")});return false});e.droppable(i._DroppableConfigListItems(e));e.find(".assigned-tag-delete").bind("click",function(f){var j=$(f.target).parent(".assigned-tag");
...
_AddListToDOM:function(b,c){this.listlist=$('<div id="list-list"></div>');this.wrapper.append(this.listlist);for(var i=0;i<b.length;i++)this.listlist.append(this._GetListElement(b[i],c));this._AdjustHeight();this._triggerResize()},_SelectLastList:function(){this.selectedList?this.selectedList.element.effect("highlight",{},300,function(){$(this).show();$(this).focus()}):this.listlist.find(".row").first().effect("highlight",{},300,function(){$(this).show();$(this).focus()})},_RegisterGlobalKeyboardShortcuts:function(){var b=
this;$(document).bind("keydown","ctrl+l",function(){b._SelectLastList()});$(document).bind("keydown","ctrl+f",function(){b.toolbar.find("#search-list").focus()});$(document).bind("keydown","shift+f",function(){b.toolbar.find("#search-list").focus();return false})},_ShowListView:function(b,c){var i=this;i.listlist.show("slide",{direction:"left"},"slow",function(){if(b){var e=i._GetListElement(b,c);i._IsNewList(b)?i.listlist.prepend(e):i.selectedList.element.replaceWith(e);i._AdjustHeight();i.selectedList=
{data:b,element:e}}i.selectedList&&i.selectedList.element.focus()})},_HideForm:function(b,c){var i=this;i.listForm.hide("slide",{direction:"left"},"slow",function(){i._ShowListView(b,c);i.listForm.trigger("FormHidden");i.selectedList?i.selectedList.element.focus():i.listlist.find(".row").first().focus();i.listForm.remove();i.listForm=null})},_Filter:function(){this._FilterBySearchText();this._FilterByTags()},_FilterBySearchText:function(){var b=this.toolbar.find("#search-list").val();if(b==="")this.listlist.find(".row").show();
else{this.listlist.find(".row:visible:not(:Contains('"+b+"'))").hide();this.listlist.find(".row:visible:Contains('"+b+"')").show()}this._AdjustHeight();this._triggerResize()},_FilterByTags:function(){var b=this;b.listlist.find(".row:visible").each(function(){var c=$(this),i=c.data("data"),e=0;if(b.selectedTags.length>0){for(var f=0;f<i.list.tags.length;f++)for(var j=0;j<b.selectedTags.length;j++)b.selectedTags[j]===i.list.tags[f].id&&e++;e===b.selectedTags.length?c.show():c.hide()}})},_create:function(){var b=
this;b.selectedList=null;b.wrapper=b.element.find("div#list-wrapper");b.toolbar=b._CreateToolbar();b.listForm=null;b.selectedTags=[];List.Index({successCallback:function(c,i){b.wrapper.find(".row").remove();b._AddListToDOM(i,c);b.listlist.find(".row").first().focus()}});b._RegisterGlobalKeyboardShortcuts()},options:{},SetupDroppable:function(b){var c=this;c.listlist.find(".row").each(function(){var i=$(this);i.droppable("destroy");switch(b){case "listitem":i.droppable(c._DroppableConfigListItems(i));
break;case "tag":i.droppable(c._DroppableConfigTags())}})},ReloadLists:function(){var b=this;List.Index({successCallback:function(c,i){b.wrapper.find(".row").remove();b._AddListToDOM(i,c);b.toolbar.find("#search-list").trigger("keyup");b._Filter()}})},SelectList:function(){this._SelectLastList()},Filter:function(b){this.selectedTags=b;this._Filter()},destroy:function(){this.toolbar.remove();this.listForm.remove();this.wrapper.find(".row").remove()}})})();
...
(function(){$.widget("ui.TagView",{_GetTag:function(b,c){var i=this,e=$($.mustache(c,b.tag));e.data("data",b);if(!i.tagMenu){i.tagMenu=e.find(".tag-menu");i.tagMenu.hide()}e.find(".tag-menu").remove();e.find(".color-selector").bind("click",function(f){var j=i.tagMenu;f=$(f.target);if(j.data("visible")===true&&j.data("tagId")===b.tag.id){j.hide("fast");j.data("visible",false);j.data("tagId",null);j.data("tag",null);j.data("target",null)}else{j.show("fast");j.css({left:e.position().left,top:e.position().top-
100});j.data("visible",true);j.data("tagId",b.tag.id);j.data("tag",b);j.data("target",f)}return false});ListKungFu&&ListKungFu.LayoutWest&&e.draggable({helper:"clone",appendTo:ListKungFu.LayoutWest,revert:"invalid",containment:ListKungFu.LayoutWest,start:function(){document.onselectstart=function(){return false};i._trigger("StartedDraggingTag",0,{dragType:"tag"})},stop:function(){document.onselectstart=null}});e.bind("click",function(){var f=e.data("data");if(e.data("selected")){i._RemoveSelectedTag(f.tag.id);
e.removeClass("tag-selected");e.data("selected",false)}else{i._AddSelectedTag(f.tag.id);e.addClass("tag-selected");e.data("selected",true)}i._trigger("TagSelected",0,i.selectedTags);return false});return e},_AddSelectedTag:function(b){this.selectedTags.push(b);this.selectedTags=jQuery.unique(widget.selectedTags)},_RemoveSelectedTag:function(b){for(var c=[],i=0;i<this.selectedTags.length;i++)this.selectedTags[i]!==b&&c.push(this.selectedTags[i]);this.selectedTags=c},_create:function(){widget=this;
widget.toolbar=$('<div id="tag-toolbar"></div>');widget.addTagInput=$('<input type="text" value="" id="add-tag"/>');widget.addTagButton=$('<button id="tag-new">Create Tag</button>');widget.tagList=$('<div class="tags"></div>');widget.element.append(widget.toolbar);widget.toolbar.append(widget.addTagInput);widget.toolbar.append(widget.addTagButton);widget.element.append(widget.tagList);widget.addTagButton.button({text:false,icons:{primary:"ui-icon-plusthick"}});widget.selectedTags=[];Tag.Index({successCallback:function(b,
c){for(var i=0;i<c.length;i++)widget.tagList.append(widget._GetTag(c[i],b));widget.tagList.append(widget.tagMenu);widget.tagMenu.find(".color").bind("click",function(e){var f=$(e.target).attr("data-colorclass");e=widget.tagMenu.data("tag");var j=e.tag.color_class,m=widget.tagMenu.data("target");e.tag.color_class=f;Tag.Update({send:e,successCallback:function(s,u){widget._trigger("AfterColorChanged",0,{});m.parent(".tag").removeClass(j);m.parent(".tag").addClass(f);m.removeClass(j);m.addClass(f);widget.tagMenu.hide("fast");
widget.tagMenu.data("visible",false);widget.tagMenu.data("tagId",null);widget.tagMenu.data("tag",null);widget.tagMenu.data("target",null);m.data("data",u)},tags:e.tag.id})});widget.tagMenu.find(".delete-label").bind("click",function(){var e=widget.tagMenu.data("tag"),f=widget.tagMenu.data("target");widget.tagMenu.hide();var j=function(){Tag.Destroy({successCallback:function(){f.hide("fast",function(){f.parent(".tag").remove()})},tags:e.tag.id})};if(typeof widget.deleteDialog==="undefined")widget.deleteDialog=
$.confirmationDialog("Delete Tag",j,"Cancel","Delete Tag and remove it from all lists?");widget.deleteDialog.dialog("open");return false})}});widget.addTagButton.bind("click",function(){if($.trim(widget.addTagInput.val())!==""&&widget.tagList.find(".tag-name:HasExactValue('"+$.trim(widget.addTagInput.val())+"')").length===0){var b={};b.tag={name:widget.addTagInput.val(),color_class:"c1"};Tag.Create({send:b,successCallback:function(c,i){var e=widget._GetTag(i,c);widget.tagList.prepend(e);widget.addTagInput.val("");
widget.addTagInput.trigger("keyup")}});return false}});widget.addTagInput.bind("keyup","return",function(){widget.addTagButton.trigger("click")});widget.addTagInput.bind("keyup",function(){var b=$(this).val();if(b==="")widget.tagList.find(".tag").show();else{widget.tagList.find(".tag:not(:Contains('"+b+"'))").hide();widget.tagList.find(".tag:Contains('"+b+"')").show()}widget.tagList.find(".tag-name:HasExactValue('"+$.trim(b)+"')").length>0?widget.addTagButton.button("option","disabled",true):widget.addTagButton.button("option",
"disabled",false)})},options:{},destroy:function(){widget.element.children().remove()}})})();
