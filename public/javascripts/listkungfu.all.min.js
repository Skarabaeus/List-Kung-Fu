...
var Controller=function(b){var c={};c.ClearFlash=function(){c.flash={notice:"",error:"",warning:"",message:""}};c.ClearFlash();c.SetFlash=function(e){c.flash.notice=e.getResponseHeader("X-Flash-Notice")||"";c.flash.error=e.getResponseHeader("X-Flash-Error")||"";c.flash.warning=e.getResponseHeader("X-Flash-Warning")||"";c.flash.message=e.getResponseHeader("X-Flash-Message")||"";if(typeof b.onFlashUpdate==="function")if(c.flash.notice!==""||c.flash.error!==""||c.flash.warning!==""||c.flash.message!==
"")b.onFlashUpdate(c.flash.notice,c.flash.error,c.flash.warning,c.flash.message)};c.baseURL=b.base_url||document.location.protocol+"//"+document.location.hostname+(document.location.port!==""?":"+document.location.port:"")+"/";c.route=b.route;c.DefaultCallback=function(e,g,f,j){c.SetFlash(j);var m=g.data,x="";if(g.template&&g.template.trim()!=="")x=$("#"+g.template).html();g=g.errors;if(!$.isArray(m))for(var y in m){m[y].isSaved=m[y].id&&m[y].id!==null?true:false;m[y].isNew=m[y].id&&m[y].id!==null?
false:true}typeof e==="function"&&e(x,m,f,j,g);c.ClearFlash()};c.DefaultErrorCallback=function(){$('<div id="dialog-confirm"> \t\t\t<span class="ui-icon ui-icon-alert" style="float:left; margin:0 7px 20px 0;"> \t\t\t</span>Something went wrong :-( Either try again or reload the page!</div>').dialog({resizable:false,modal:true,buttons:{"Try again":function(){$(this).dialog("close")},Reload:function(){window.location.reload()}}})};c.ConstructRoute=function(e){for(var g="",f="",j=0;j<c.route.length;j++){g+=
c.route[j];f=e[c.route[j]]||"";if(f!=="")g+="/"+f;if(j!==c.route.length-1)g+="/"}return g};c.Index=function(e){var g=e.successCallback||null,f=c.ConstructRoute(e);$.ajax({url:c.baseURL+f,dataType:"json",type:"GET",processData:true,contentType:"application/json",data:e.send,beforeSend:function(j){j.setRequestHeader("X-CSRF-Token",$('meta[name="csrf-token"]').attr("content"))},success:function(j,m,x){c.DefaultCallback(g,j,m,x)},error:function(j,m,x){c.DefaultErrorCallback(j,m,x)}})};c.Show=function(e){var g=
e.successCallback||null;e=c.ConstructRoute(e);$.ajax({url:c.baseURL+e,dataType:"json",type:"GET",processData:false,contentType:"application/json",beforeSend:function(f){f.setRequestHeader("X-CSRF-Token",$('meta[name="csrf-token"]').attr("content"))},success:function(f,j,m){c.DefaultCallback(g,f,j,m)},error:function(f,j,m){c.DefaultErrorCallback(f,j,m)}})};c.New=function(e){var g=e.successCallback||null;e=c.ConstructRoute(e);$.ajax({url:c.baseURL+e+"/new",dataType:"json",type:"GET",processData:false,
contentType:"application/json",beforeSend:function(f){f.setRequestHeader("X-CSRF-Token",$('meta[name="csrf-token"]').attr("content"))},success:function(f,j,m){c.DefaultCallback(g,f,j,m)},error:function(f,j,m){c.DefaultErrorCallback(f,j,m)}})};c.Create=function(e){var g=e.successCallback||null,f=c.ConstructRoute(e);e=JSON.stringify(e.send);$.ajax({url:c.baseURL+f,dataType:"json",type:"POST",processData:false,contentType:"application/json",data:e,beforeSend:function(j){j.setRequestHeader("X-CSRF-Token",
$('meta[name="csrf-token"]').attr("content"))},success:function(j,m,x){c.DefaultCallback(g,j,m,x)},error:function(j,m,x){c.DefaultErrorCallback(j,m,x)}})};c.Edit=function(e){var g=e.successCallback||null;e=c.ConstructRoute(e);$.ajax({url:c.baseURL+e+"/edit",dataType:"json",type:"GET",parseData:false,contentType:"application/json",beforeSend:function(f){f.setRequestHeader("X-CSRF-Token",$('meta[name="csrf-token"]').attr("content"))},success:function(f,j,m){c.DefaultCallback(g,f,j,m)},error:function(f,
j,m){c.DefaultErrorCallback(f,j,m)}})};c.Update=function(e){var g=e.successCallback||null,f=c.ConstructRoute(e);e=JSON.stringify(e.send);e=e.replace(/\\u000a/g,"\\n");$.ajax({url:c.baseURL+f,dataType:"json",type:"POST",processData:false,contentType:"application/json",data:e,beforeSend:function(j){j.setRequestHeader("X-Http-Method-Override","PUT");j.setRequestHeader("X-CSRF-Token",$('meta[name="csrf-token"]').attr("content"))},success:function(j,m,x){c.DefaultCallback(g,j,m,x)},error:function(j,m,
x){c.DefaultErrorCallback(j,m,x)}})};c.Destroy=function(e){var g=e.successCallback||null;e=c.ConstructRoute(e);$.ajax({url:c.baseURL+e,dataType:"json",type:"POST",processData:false,contentType:"application/json",beforeSend:function(f){f.setRequestHeader("X-Http-Method-Override","DELETE");f.setRequestHeader("X-CSRF-Token",$('meta[name="csrf-token"]').attr("content"))},success:function(f,j,m){c.DefaultCallback(g,f,j,m)},error:function(f,j,m){c.DefaultErrorCallback(f,j,m)}})};return c},List=Controller({route:["lists"],
onFlashUpdate:function(b,c){ListKungFu.LayoutSouth.StatusBar("SetAlert",c);ListKungFu.LayoutSouth.StatusBar("SetNotice",b)}}),ListListItem=Controller({route:["lists","list_items"],onFlashUpdate:function(b,c){ListKungFu.LayoutSouth.StatusBar("SetAlert",c);ListKungFu.LayoutSouth.StatusBar("SetNotice",b)}}),ListItem=Controller({route:["list_items"],onFlashUpdate:function(b,c){ListKungFu.LayoutSouth.StatusBar("SetAlert",c);ListKungFu.LayoutSouth.StatusBar("SetNotice",b)}}),Tag=Controller({route:["tags"],
onFlashUpdate:function(b,c){ListKungFu.LayoutSouth.StatusBar("SetAlert",c);ListKungFu.LayoutSouth.StatusBar("SetNotice",b)}});
jQuery(function(b){var c=b("meta[name=csrf-token]").attr("content"),e=b("meta[name=csrf-param]").attr("content");b.fn.extend({triggerAndReturn:function(f,j){var m=new b.Event(f);this.trigger(m,j);return m.result!==false},callRemote:function(){var f=this,j=f.attr("method")||f.attr("data-method")||"GET",m=f.attr("action")||f.attr("href"),x=f.attr("data-type")||"script";if(m===undefined)throw"No URL specified for remote call (action or href must be present).";else{if(f.triggerAndReturn("ajax:before")){var y=
f.is("form")?f.serializeArray():[];b.ajax({url:m,data:y,dataType:x,type:j.toUpperCase(),beforeSend:function(p){f.trigger("ajax:loading",p)},success:function(p,E,w){f.trigger("ajax:success",[p,E,w])},complete:function(p){f.trigger("ajax:complete",p)},error:function(p,E,w){f.trigger("ajax:failure",[p,E,w])}})}f.trigger("ajax:after")}}});b("body").delegate("a[data-confirm], button[data-confirm], input[data-confirm]","click.rails",function(){var f=b(this);if(f.triggerAndReturn("confirm"))if(!confirm(f.attr("data-confirm")))return false});
...
(function(){$.widget("ui.Dashboard",{_TriggerResize:function(){this._trigger("ContentDimensionsChanged",0,{})},_NothingToDoText:function(){return'<p class="nothing-todo">no items scheduled</p>'},_TriggerReinitOfPanes:function(){this._trigger("ReinitPanes",0,{})},_SetupCustomDeadlinePicker:function(b){var c=this,e=$(this),g=b.find(".dashboard-list-item-deadline-hidden"),f=b.data("data"),j=f.list_item.deadline_date.split("-"),m=j[0],x=j[1],y=j[2];g.datepicker({minDate:new Date,dateFormat:"yy,mm,dd",
beforeShow:function(p,E){E.input.datepicker("setDate",m+","+x+","+y)},onSelect:function(p){f.list_item.deadlineType="customdeadline";f.list_item.customDeadlineValue=p;f.list_item.template="dashboard";ListListItem.Update({successCallback:function(E,w){var F=b.parent();b.hide("slow",function(){b.remove();F.find(".dashboard-item").length===0&&F.append(c._NothingToDoText())});c._CreateDashboardItem(w,E,{highlightCategory:true,addType:"prepend"})},lists:f.list_item.list_id,list_items:f.list_item.id,send:f});
e.datepicker("hide")}})},_AppendToDashboard:function(b,c,e){e=e||{};b.find(".nothing-todo").remove();e.addType==="prepend"?b.find("div[data-type=scheduleColumn]").first().prepend(c):b.find("div[data-type=scheduleColumn]").first().append(c);e.highlightCategory===true&&c.effect("highlight",{},2E3)},_CreateDashboardItem:function(b,c,e){var g=this,f=$($.mustache(c,b.list_item));switch(b.list_item.deadline_category){case "today":g._AppendToDashboard(g.today,f,e);break;case "tomorrow":g._AppendToDashboard(g.tomorrow,
f,e);break;case "this week":g._AppendToDashboard(g.thisweek,f,e);break;case "next week":g._AppendToDashboard(g.nextweek,f,e);break;default:g._AppendToDashboard(g.later,f,e)}f.data("data",b);f.find(".dashboard-list-item-title").bind("click",function(){g._trigger("OpenList",0,{selectedList:b.list_item})});f.find(".dashboard-item-more").bind("click",function(){g._trigger("OpenList",0,{selectedList:b.list_item})});f.find(".full-content").bind("dblclick",function(){g._trigger("OpenListItem",0,{selectedListItem:b})});
f.find(".shortend-content").bind("dblclick",function(){g._trigger("OpenListItem",0,{selectedListItem:b})});f.find(".full-content").bind("click",function(){$(this).hide();f.find(".shortend-content").show()});f.find(".shortend-content").bind("click",function(){$(this).hide();f.find(".full-content").show()});f.find(".dashboard-item-completed").bind("click",function(){$that=$(this);$that.find(".confirmation-completed").remove();var j=$('<div class="confirmation-completed">Completed? \t\t\t\t\t<span class="yes">Yes</span> / <span class="no">NO</span></div>');
...
e){$.each(e,function(g,f){b._CreateDashboardItem(f,c)});$('div[data-type="scheduleColumn"]').each(function(){$(this).children(".dashboard-item").length===0&&$(this).append(b._NothingToDoText())});b._TriggerReinitOfPanes();b._TriggerResize()},send:{show:"dashboard"}})},Hide:function(){this.element.children().remove()},destroy:function(){},Filter:function(b){this.selectedText=b;this._Filter()}})})();
(function(b){function c(e){if(typeof e.data==="string"){var g=e.handler,f=e.data.toLowerCase().split(" ");e.handler=function(j){if(!(this!==j.target&&(/textarea|select/i.test(j.target.nodeName)||j.target.type==="text"))){var m=j.type!=="keypress"&&b.hotkeys.specialKeys[j.which],x=String.fromCharCode(j.which).toLowerCase(),y="",p={};if(j.altKey&&m!=="alt")y+="alt+";if(j.ctrlKey&&m!=="ctrl")y+="ctrl+";if(j.metaKey&&!j.ctrlKey&&m!=="meta")y+="meta+";if(j.shiftKey&&m!=="shift")y+="shift+";if(m)p[y+m]=
true;else{p[y+x]=true;p[y+b.hotkeys.shiftNums[x]]=true;if(y==="shift+")p[b.hotkeys.shiftNums[x]]=true}m=0;for(x=f.length;m<x;m++)if(p[f[m]])return g.apply(this,arguments)}}}}b.hotkeys={version:"0.8",specialKeys:{8:"backspace",9:"tab",13:"return",16:"shift",17:"ctrl",18:"alt",19:"pause",20:"capslock",27:"esc",32:"space",33:"pageup",34:"pagedown",35:"end",36:"home",37:"left",38:"up",39:"right",40:"down",45:"insert",46:"del",96:"0",97:"1",98:"2",99:"3",100:"4",101:"5",102:"6",103:"7",104:"8",105:"9",
...
function(m){m.preventDefault();var x=f.serializeForm();x.list_item.deadlineType=$(m.target).attr("data-deadline");x.list_item.customDeadlineValue=$("#custom-deadline-value").val();ListListItem.Create({send:x,successCallback:function(y,p){f.hide("slow",function(){b._AddListItem(p,y,true);$(this).remove();b.toolbar.find("#list-item-search").trigger("ClearValue");b.listItemList.find(".row").show();b.listItemList.find(".row").first().focus();b.listItemList.find(".row").first().find(".handle").height(b.listItemList.find(".row").first().find(".list-item-content").height())})},
...
j=f.find("ul"),m=$('*[name*="'+e+'"]'),x=0;x<g.length;x++){m.parent(".field").addClass("field_with_errors");j.append("<li>"+g[x]+"</li>")}m.parent(".field").append(f)}},_ClearFormErrors:function(b){b.find(".field").removeClass("field_with_errors");b.find(".error_explanation").remove()},_DeleteList:function(b){var c=this,e=c.selectedList.element.next(),g=c.selectedList.element.prev();b=b||0;List.Destroy({successCallback:function(){c.selectedList.element.delay(b).fadeOut(function(){$(this).remove();
...
$(this),e=c.data("data"),g=0;if(b.selectedTags.length>0){for(var f=0;f<e.list.tags.length;f++)for(var j=0;j<b.selectedTags.length;j++)b.selectedTags[j]===e.list.tags[f].id&&g++;g===b.selectedTags.length?c.show():c.hide()}})},_create:function(){var b=this;b.selectedList=null;b.wrapper=b.element.find("div#list-wrapper");b.toolbar=b._CreateToolbar();b.listForm=null;b.selectedTags=[];var c=new Date;c=Date.UTC(c.getFullYear(),c.getMonth(),c.getDay(),c.getUTCHours(),c.getUTCMinutes(),c.getUTCSeconds(),
c.getUTCMilliseconds());b.emptyListImage=$('<img src="/images/empty/list.jpg?'+c+'"/>');b.emptyListImage.css({position:"absolute",top:"45px",left:"15px"});b.emptyListImage.hide();b.toolbar.append(b.emptyListImage);List.Index({successCallback:function(e,g){b.wrapper.find(".row").remove();b._AddListToDOM(g,e);b._ToggleEmptyListImage()}});b._RegisterGlobalKeyboardShortcuts()},_ToggleEmptyListImage:function(){if(this.listlist.find(".row").length>0){this.emptyListImage.hide();this.listlist.find(".row").first().focus()}else this.emptyListImage.show()},
options:{},SetupDroppable:function(b){var c=this;c.listlist.find(".row").each(function(){var e=$(this);e.droppable("destroy");switch(b){case "listitem":e.droppable(c._DroppableConfigListItems(e));break;case "tag":e.droppable(c._DroppableConfigTags())}})},ReloadLists:function(){var b=this;List.Index({successCallback:function(c,e){b.wrapper.find(".row").remove();b._AddListToDOM(e,c);b._Filter();b._ToggleEmptyListImage()}})},SelectList:function(){this._SelectLastList()},Filter:function(b,c){if(b)this.selectedTags=
b;if(!(c==null||typeof c==="undefined"))this.selectedText=c;this._Filter()},destroy:function(){this.toolbar.remove();this.listForm.remove();this.wrapper.find(".row").remove()}})})();(function(b){b.fn.serializeForm=function(){var c=b(this).find('*[name*="]"]'),e={};b.each(c,function(g,f){var j=b(f),m=j.get(0).name.split("[")[0],x=j.get(0).name.split("[")[1];x=x.replace("]","");j=j.val();e[m]=e[m]||{};e[m][x]=j});return e}})(jQuery);
...
(function(){function b(p){return p<10?"0"+p:p}function c(p){f.lastIndex=0;return f.test(p)?'"'+p.replace(f,function(E){var w=x[E];return typeof w==="string"?w:"\\u"+("0000"+E.charCodeAt(0).toString(16)).slice(-4)})+'"':'"'+p+'"'}function e(p,E){var w,F,H,I,C=j,K,D=E[p];if(D&&typeof D==="object"&&typeof D.toJSON==="function")D=D.toJSON(p);if(typeof y==="function")D=y.call(E,p,D);switch(typeof D){case "string":return c(D);case "number":return isFinite(D)?String(D):"null";case "boolean":case "null":return String(D);
case "object":if(!D)return"null";j+=m;K=[];if(Object.prototype.toString.apply(D)==="[object Array]"){I=D.length;for(w=0;w<I;w+=1)K[w]=e(w,D)||"null";H=K.length===0?"[]":j?"[\n"+j+K.join(",\n"+j)+"\n"+C+"]":"["+K.join(",")+"]";j=C;return H}if(y&&typeof y==="object"){I=y.length;for(w=0;w<I;w+=1){F=y[w];if(typeof F==="string")if(H=e(F,D))K.push(c(F)+(j?": ":":")+H)}}else for(F in D)if(Object.hasOwnProperty.call(D,F))if(H=e(F,D))K.push(c(F)+(j?": ":":")+H);H=K.length===0?"{}":j?"{\n"+j+K.join(",\n"+j)+
...
f=/[\\\"\x00-\x1f\x7f-\x9f\u00ad\u0600-\u0604\u070f\u17b4\u17b5\u200c-\u200f\u2028-\u202f\u2060-\u206f\ufeff\ufff0-\uffff]/g,j,m,x={"\u0008":"\\b","\t":"\\t","\n":"\\n","\u000c":"\\f","\r":"\\r",'"':'\\"',"\\":"\\\\"},y;if(typeof JSON.stringify!=="function")JSON.stringify=function(p,E,w){var F;m=j="";if(typeof w==="number")for(F=0;F<w;F+=1)m+=" ";else if(typeof w==="string")m=w;if((y=E)&&typeof E!=="function"&&(typeof E!=="object"||typeof E.length!=="number"))throw Error("JSON.stringify");return e("",
{"":p})};if(typeof JSON.parse!=="function")JSON.parse=function(p,E){function w(H,I){var C,K,D=H[I];if(D&&typeof D==="object")for(C in D)if(Object.hasOwnProperty.call(D,C)){K=w(D,C);if(K!==undefined)D[C]=K;else delete D[C]}return E.call(H,I,D)}var F;p=String(p);g.lastIndex=0;if(g.test(p))p=p.replace(g,function(H){return"\\u"+("0000"+H.charCodeAt(0).toString(16)).slice(-4)});if(/^[\],:{}\s]*$/.test(p.replace(/\\(?:["\\\/bfnrt]|u[0-9a-fA-F]{4})/g,"@").replace(/"[^"\\\n\r]*"|true|false|null|-?\d+(?:\.\d*)?(?:[eE][+\-]?\d+)?/g,
"]").replace(/(?:^|:|,)(?:\s*\[)+/g,""))){F=eval("("+p+")");return typeof E==="function"?w({"":F},""):F}throw new SyntaxError("JSON.parse");}})();
...
e,g){if(!this.includes("#",c)&&!this.includes("^",c))return c;var f=this;return c.replace(RegExp(this.otag+"(\\^|\\#)\\s*(.+)\\s*"+this.ctag+"\n*([\\s\\S]+?)"+this.otag+"\\/\\s*\\2\\s*"+this.ctag+"\\s*","mg"),function(j,m,x,y){j=f.find(x,e);if(m=="^")return!j||f.is_array(j)&&j.length===0?f.render(y,e,g,true):"";else if(m=="#")return f.is_array(j)?f.map(j,function(p){return f.render(y,f.create_context(p),g,true)}).join(""):f.is_object(j)?f.render(y,f.create_context(j),g,true):typeof j==="function"?
j.call(e,y,function(p){return f.render(p,e,g,true)}):j?f.render(y,e,g,true):""})},render_tags:function(c,e,g,f){var j=this,m=function(){return RegExp(j.otag+"(=|!|>|\\{|%)?([^\\/#\\^]+?)\\1?"+j.ctag+"+","g")},x=m(),y=function(E,w,F){switch(w){case "!":return"";case "=":j.set_delimiters(F);x=m();return"";case ">":return j.render_partial(F,e,g);case "{":return j.find(F,e);default:return j.escape(j.find(F,e))}};c=c.split("\n");for(var p=0;p<c.length;p++){c[p]=c[p].replace(x,y,this);f||this.send(c[p])}if(f)return c.join("\n")},
...
(function(b){b.fn.layout=function(c){function e(d){if(!d)return true;var h=d.keyCode;if(h<33)return true;var i={38:"north",40:"south",37:"west",39:"east"},k=h>=37&&h<=40,l=d.shiftKey,n=d.ctrlKey,s=false,q,u,r;if(!n&&!l)return true;else if(k&&p[i[h]].enableCursorHotkey)s=i[h];else b.each(t.borderPanes.split(","),function(v,A){q=p[A];u=q.customHotkey;r=q.customHotkeyModifier;if(l&&r=="SHIFT"||n&&r=="CTRL"||n&&l)if(u&&h==(isNaN(u)||u<=9?u.toUpperCase().charCodeAt(0):u)){s=A;return false}});if(!s)return true;
q=p[s];i=w[s];if(!q.enableCursorHotkey||i.isHidden||!B[s])return true;if((i=d.target||d.srcElement)&&l&&k&&(i.tagName=="TEXTAREA"||i.tagName=="INPUT"&&(h==37||h==39)))return true;ba(s);d.stopPropagation();return d.returnValue=false}function g(d){if(this&&this.tagName)d=this;d=typeof d=="string"?B[d]:b(d).attr("pane")?b(d):b(d).parents("div[pane]:first");if(d.length){var h=d.attr("pane"),i=w[h];i.cssSaved&&f(h);if(i.isSliding||i.isResizing||i.isClosed)i.cssSaved=false;else{var k={zIndex:t.zIndex.pane_normal+
...
resizerCursor:"e-resize"},west:{paneSelector:".ui-layout-west",size:200,resizerCursor:"w-resize"},center:{paneSelector:".ui-layout-center"}},E={slide:{all:{duration:"fast"},north:{direction:"up"},south:{direction:"down"},east:{direction:"right"},west:{direction:"left"}},drop:{all:{duration:"slow"},north:{direction:"up"},south:{direction:"down"},east:{direction:"right"},west:{direction:"left"}},scale:{all:{duration:"fast"}}},w={id:Math.floor(Math.random()*1E4),container:{},north:{},south:{},east:{},
...
d.defaults||d.north||d.south||d.west||d.east||d.center)h=b.extend(h,d);else b.each(d,function(i,k){a=i.split("__");h[a[1]?a[0]:"defaults"][a[1]?a[1]:a[0]]=k});return h},D=function(d,h,i){function k(s){n=t[s];if(n.doCallback){cpPane=n.callback.split(",")[1];cpPane!=s&&cpPane!=h&&k(cpPane,true)}else{n.doCallback=true;n.callback=l}}var l=d+","+h+","+(i?1:0),n;b.each(t.borderPanes.split(","),function(s,q){if(t[q].isMoving){k(q);return false}})},la=function(d){d=t[d];t.isLayoutBusy=false;delete d.isMoving;
if(d.doCallback&&d.callback){d.doCallback=false;var h=d.callback.split(","),i=h[2]>0?true:false;if(h[0]=="open")V(h[1],i);else h[0]=="close"&&T(h[1],i);if(!d.doCallback)d.callback=null}},L=function(d,h){if(h){var i;try{if(typeof h=="function")i=h;else if(typeof h!="string")return;else if(h.indexOf(",")>0){var k=h.split(",");i=eval(k[0]);if(typeof i=="function"&&k.length>1)return i(k[1])}else i=eval(h);if(typeof i=="function")return i(d,B[d],b.extend({},w[d]),b.extend({},p[d]),p.name)}catch(l){}}},
...
true)=="none"?0:Q(i,"borderLeftWidth"))-(b.curCSS(i[0],"borderRightStyle",true)=="none"?0:Q(i,"borderRightWidth")):h},N=function(d,h){var i;if(H(d)){d=I(d);i=B[d]}else i=b(d);if(h<=0)return 0;else h>0||(h=H(d)?S(d):i.outerHeight());return b.boxModel?h-Q(i,"paddingTop")-Q(i,"paddingBottom")-(b.curCSS(i[0],"borderTopStyle",true)=="none"?0:Q(i,"borderTopWidth"))-(b.curCSS(i[0],"borderBottomStyle",true)=="none"?0:Q(i,"borderBottomWidth")):h},S=function(d,h){var i=B[d],k=p[d],l=w[d],n=h?k.spacing_open:
0;k=h?k.spacing_closed:0;return!i||l.isHidden?0:l.isClosed||l.isSliding&&h?k:t[d].dir=="horz"?i.outerHeight()+n:i.outerWidth()+n},ca=function(d){var h=z,i=t[d].dir,k=w[d],l=B[F[d]],n=p[d].spacing_open,s=p[F[d]].spacing_open,q=!l?0:i=="horz"?l.outerHeight():l.outerWidth();l=k.minSize||0;i=Math.min(k.maxSize||9999,(i=="horz"?h.innerHeight:h.innerWidth)-n-q-s);var u,r;switch(d){case "north":u=h.offsetTop+l;r=h.offsetTop+i;break;case "west":u=h.offsetLeft+l;r=h.offsetLeft+i;break;case "south":u=h.offsetTop+
...
function(){i=I(this);k=h["border"+i]=Q(d,"border"+i+"Width");l=h["padding"+i]=Q(d,"padding"+i);h["offset"+i]=k+l;if(d==M)h[i.toLowerCase()]=b.boxModel?l:0});h.innerWidth=h.outerWidth=d.outerWidth();h.innerHeight=h.outerHeight=d.outerHeight();if(b.boxModel){h.innerWidth-=h.offsetLeft+h.offsetRight;h.innerHeight-=h.offsetTop+h.offsetBottom}return h},ra=function(d,h,i,k){var l=window.layout=window.layout||{};l=l.timers=l.timers||{};d="layout_"+w.id+"_"+d+"_"+h;l[d]||(l[d]=setTimeout(i,k))},da=function(d,
h){var i=window.layout=window.layout||{};i=i.timers=i.timers||{};var k="layout_"+w.id+"_"+d+"_"+h;if(i[k]){clearTimeout(i[k]);delete i[k];return true}else return false},sa=function(){try{if(M[0].tagName=="BODY"){b("html").css({height:"100%",overflow:"hidden"});b("body").css({position:"relative",height:"100%",overflow:"hidden",margin:0,padding:0,border:"none"})}else{var d={overflow:"hidden"},h=M.css("position"),i=M.css("height");if(!M.hasClass("ui-layout-pane")){if(!h||"fixed,absolute,relative".indexOf(h)<
0)d.position="relative";if(!i||i=="auto")d.height="100%"}M.css(d)}}catch(k){}z=w.container=na(M)},ta=function(){b.each(t.borderPanes.split(","),function(d,h){var i=p[h];if(i.enableCursorHotkey||i.customHotkey){b(document).keydown(e);return false}})},ua=function(){c=K(c);if(c.effects){b.extend(E,c.effects);delete c.effects}b.each("name,scrollToBookmarkOnLoad".split(","),function(i,k){if(c[k]!==undefined)p[k]=c[k];else if(c.defaults[k]!==undefined){p[k]=c.defaults[k];delete c.defaults[k]}});b.each("paneSelector,resizerCursor,customHotkey".split(","),
function(i,k){delete c.defaults[k]});b.extend(p.defaults,c.defaults);t.center=b.extend(true,{},t.defaults,t.center);b.extend(p.center,c.center);var d=b.extend(true,{},p.defaults,c.defaults,p.center);b.each("paneClass,contentSelector,contentIgnoreSelector,applyDefaultStyles,showOverflowOnHover".split(","),function(i,k){p.center[k]=d[k]});var h=p.defaults;b.each(t.borderPanes.split(","),function(i,k){t[k]=b.extend(true,{},t.defaults,t[k]);o=p[k]=b.extend(true,{},p.defaults,p[k],c.defaults,c[k]);if(!o.paneClass)o.paneClass=
y.paneClass;if(!o.resizerClass)o.resizerClass=y.resizerClass;if(!o.togglerClass)o.togglerClass=y.togglerClass;b.each(["_open","_close",""],function(l,n){var s="fxName"+n,q="fxSpeed"+n,u="fxSettings"+n;o[s]=c[k][s]||c[k].fxName||c.defaults[s]||c.defaults.fxName||o[s]||o.fxName||h[s]||h.fxName||"none";var r=o[s];if(r=="none"||!b.effects||!b.effects[r]||!E[r]&&!o[u]&&!o.fxSettings)r=o[s]="none";r=E[r]||{};s=r.all||{};r=r[k]||{};o[u]=b.extend({},s,r,h.fxSettings||{},h[u]||{},o.fxSettings,o[u],c.defaults.fxSettings,
c.defaults[u]||{},c[k].fxSettings,c[k][u]||{});o[q]=c[k][q]||c[k].fxSpeed||c.defaults[q]||c.defaults.fxSpeed||o[q]||o[u].duration||o.fxSpeed||o.fxSettings.duration||h.fxSpeed||h.fxSettings.duration||r.duration||s.duration||"normal"})})},oa=function(){b.each(t.allPanes.split(","),function(){var d=I(this),h=p[d],i=w[d],k=t[d].dir,l=h.size=="auto"||isNaN(h.size)?0:h.size,n=h.minSize||1,s=h.maxSize||9999,q=h.spacing_open||0,u=h.paneSelector,r={},v;X[d]=false;if(u.substr(0,1)==="#")v=B[d]=M.find(u+":first");
else{v=B[d]=M.children(u+":first");v.length||(v=B[d]=M.children("form:first").children(u+":first"))}if(!v.length){B[d]=false;return true}v.attr("pane",d).addClass(h.paneClass+" "+h.paneClass+"-"+d);if(d!="center"){i.isClosed=false;i.isSliding=false;i.isResizing=false;i.isHidden=false;i.noRoom=false;t[d].pins=[]}r=b.extend({visibility:"visible",display:"block"},t.defaults.cssReq,t[d].cssReq);h.applyDefaultStyles&&b.extend(r,t.defaults.cssDef,t[d].cssDef);v.css(r);r={};switch(d){case "north":r.top=
z.top;r.left=z.left;r.right=z.right;break;case "south":r.bottom=z.bottom;r.left=z.left;r.right=z.right;break;case "west":r.left=z.left;break;case "east":r.right=z.right}if(k=="horz"){if(l===0||l=="auto"){v.css({height:"auto"});l=v.outerHeight()}l=C(l,n);l=Math.min(l,s);l=Math.min(l,z.innerHeight-q);r.height=C(1,N(d,l));i.size=l;i.maxSize=s;i.minSize=C(n,l-r.height+1);v.css(r)}else if(k=="vert"){if(l===0||l=="auto"){v.css({width:"auto","float":"left"});l=v.outerWidth();v.css({"float":"none"})}l=C(l,
n);l=Math.min(l,s);l=Math.min(l,z.innerWidth-q);r.width=C(1,R(d,l));i.size=l;i.maxSize=s;i.minSize=C(n,l-r.width+1);v.css(r);W(d,null,true)}else if(d=="center"){v.css(r);W("center",null,true)}if(h.initClosed&&h.closable){v.hide().addClass("closed");i.isClosed=true}else if(h.initHidden||h.initClosed){ga(d,true);i.isHidden=true}else v.addClass("open");h.showOverflowOnHover&&v.hover(g,f);if(h.contentSelector){i=X[d]=v.children(h.contentSelector+":first");if(!i.length){X[d]=false;return true}i.css(t.content.cssReq);
h.applyDefaultStyles&&i.css(t.content.cssDef);v.css({overflow:"hidden"})}})},va=function(){b.each(t.borderPanes.split(","),function(){var d=I(this),h=p[d],i=w[d],k=h.resizerClass,l=h.togglerClass,n=B[d];O[d]=false;Y[d]=false;if(!(!n||!h.closable&&!h.resizable)){var s=t[d].edge;n=n.is(":visible");var q="-"+d,u=n?"-open":"-closed",r;r=O[d]=b("<span></span>");n&&h.resizable||!n&&h.slidable&&r.attr("title",h.sliderTip).css("cursor",h.sliderCursor);r.attr("id",h.paneSelector.substr(0,1)=="#"?h.paneSelector.substr(1)+
"-resizer":"").attr("resizer",d).css(t.resizers.cssReq).css(s,z[s]+S(d)).addClass(k+" "+k+q+" "+k+u+" "+k+q+u).appendTo(M);h.applyDefaultStyles&&r.css(t.resizers.cssDef);if(h.closable){k=Y[d]=b("<div></div>");k.attr("id",h.paneSelector.substr(0,1)=="#"?h.paneSelector.substr(1)+"-toggler":"").css(t.togglers.cssReq).attr("title",n?h.togglerTip_open:h.togglerTip_closed).click(function(v){ba(d);v.stopPropagation()}).mouseover(function(v){v.stopPropagation()}).addClass(l+" "+l+q+" "+l+u+" "+l+q+u).appendTo(r);
if(h.togglerContent_open)b("<span>"+h.togglerContent_open+"</span>").addClass("content content-open").css("display",i.isClosed?"none":"block").appendTo(k);if(h.togglerContent_closed)b("<span>"+h.togglerContent_closed+"</span>").addClass("content content-closed").css("display",i.isClosed?"block":"none").appendTo(k);h.applyDefaultStyles&&k.css(t.togglers.cssDef);n||ha(d,true)}}});Z("all",true)},wa=function(){var d=typeof b.fn.draggable=="function",h;b.each(t.borderPanes.split(","),function(){var i=
I(this),k=p[i],l=w[i];if(!d||!B[i]||!k.resizable){k.resizable=false;return true}var n=k.resizerClass,s=n+"-drag",q=n+"-"+i+"-drag",u=n+"-dragging",r=n+"-"+i+"-dragging",v=false,A=O[i];l.isClosed||A.attr("title",k.resizerTip).css("cursor",k.resizerCursor);A.draggable({containment:M[0],axis:t[i].dir=="horz"?"y":"x",delay:200,distance:1,helper:"clone",opacity:k.resizerDragOpacity,zIndex:t.zIndex.resizing,start:function(){if(false===L(i,k.onresize_start))return false;l.isResizing=true;da(i,"closeSlider");
A.addClass(s+" "+q);v=false;var G=i=="east"||i=="south"?k.spacing_open:0;ca(i);l.minPosition-=G;l.maxPosition-=G;h=t[i].dir=="horz"?"top":"left";b(k.maskIframesOnResize===true?"iframe":k.maskIframesOnResize).each(function(){b('<div class="ui-layout-mask"/>').css({background:"#fff",opacity:"0.001",zIndex:9,position:"absolute",width:this.offsetWidth+"px",height:this.offsetHeight+"px"}).css(b(this).offset()).appendTo(this.parentNode)})},drag:function(G,J){if(!v){b(".ui-draggable-dragging").addClass(u+
" "+r).children().css("visibility","hidden");v=true;l.isSliding&&B[i].css("zIndex",t.zIndex.sliding)}if(J.position[h]<l.minPosition)J.position[h]=l.minPosition;else if(J.position[h]>l.maxPosition)J.position[h]=l.maxPosition},stop:function(G,J){var P=J.position,U;A.removeClass(s+" "+q);switch(i){case "north":U=P.top;break;case "west":U=P.left;break;case "south":U=z.outerHeight-P.top-A.outerHeight();break;case "east":U=z.outerWidth-P.left-A.outerWidth()}ia(i,U-z[t[i].edge]);b("div.ui-layout-mask").remove();
l.isResizing=false}})})},ga=function(d,h){var i=p[d],k=w[d],l=B[d],n=O[d];if(!(!l||k.isHidden))if(false!==L(d,i.onhide_start)){k.isSliding=false;n&&n.hide();if(h||k.isClosed){k.isClosed=true;k.isHidden=true;l.hide();W(t[d].dir=="horz"?"all":"center");L(d,i.onhide_end||i.onhide)}else{k.isHiding=true;T(d,false)}}},ea=function(d,h){var i=p[d],k=w[d],l=O[d];if(B[d]&&k.isHidden)if(false!==L(d,i.onshow_start)){k.isSliding=false;k.isShowing=true;l&&i.spacing_open>0&&l.show();h===false?T(d,true):V(d)}},ba=
function(d){var h=w[d];if(h.isHidden)ea(d);else h.isClosed?V(d):T(d)},T=function(d,h,i){function k(){ha(d,true);J||L(d,q.onclose_end||q.onclose);if(J)L(d,q.onshow_end||q.onshow);if(P)L(d,q.onhide_end||q.onhide);la(d)}var l=B[d],n=O[d],s=Y[d],q=p[d],u=w[d];i=!i&&!u.isClosed&&q.fxName_close!="none";var r=t[d].edge,v=q.resizerClass,A=q.togglerClass,G="-"+d,J=u.isShowing,P=u.isHiding;delete u.isShowing;delete u.isHiding;if(!(!l||!q.resizable&&!q.closable))if(!(!h&&u.isClosed&&!J))if(t.isLayoutBusy)D("close",
d,h);else if(!(!J&&false===L(d,q.onclose_start))){t[d].isMoving=true;t.isLayoutBusy=true;u.isClosed=true;if(P)u.isHidden=true;else if(J)u.isHidden=false;m(d,false);u.isSliding||W(t[d].dir=="horz"?"all":"center");if(n){n.css(r,z[r]).removeClass(v+"-open "+v+G+"-open").removeClass(v+"-sliding "+v+G+"-sliding").addClass(v+"-closed "+v+G+"-closed");q.resizable&&n.draggable("disable").css("cursor","default").attr("title","");s&&s.removeClass(A+"-open "+A+G+"-open").addClass(A+"-closed "+A+G+"-closed").attr("title",
q.togglerTip_closed);Z()}if(i){fa(d,true);l.hide(q.fxName_close,q.fxSettings_close,q.fxSpeed_close,function(){fa(d,false);u.isClosed&&k()})}else{l.hide();k()}}},V=function(d,h,i){function k(){u.isSliding||W(t[d].dir=="vert"?"center":"all");if(n){n.css(r,z[r]+S(d)).removeClass(v+P+" "+v+G+P).addClass(v+J+" "+v+G+J).addClass(!u.isSliding?"":v+U+" "+v+G+U);q.resizable?n.draggable("enable").css("cursor",q.resizerCursor).attr("title",q.resizerTip):n.css("cursor","default");s&&s.removeClass(A+P+" "+A+G+
P).addClass(A+J+" "+A+G+J).attr("title",q.togglerTip_open);Z("all")}aa(d);m(d,!u.isSliding);L(d,q.onopen_end||q.onopen);if(ja)L(d,q.onshow_end||q.onshow);la(d)}var l=B[d],n=O[d],s=Y[d],q=p[d],u=w[d];i=!i&&u.isClosed&&q.fxName_open!="none";var r=t[d].edge,v=q.resizerClass,A=q.togglerClass,G="-"+d,J="-open",P="-closed",U="-sliding",ja=u.isShowing;delete u.isShowing;if(!(!l||!q.resizable&&!q.closable))if(u.isClosed||u.isSliding)if(u.isHidden&&!ja)ea(d,true);else if(t.isLayoutBusy)D("open",d,h);else if(false!==
L(d,q.onopen_start)){t[d].isMoving=true;t.isLayoutBusy=true;u.isSliding&&!h&&ka(d,false);u.isClosed=false;if(ja)u.isHidden=false;ca(d);u.size>u.maxSize&&l.css(t[d].sizeType,C(1,t[d].dir=="horz"?N(d,u.maxSize):R(d,u.maxSize)));ha(d,false);if(i){fa(d,true);l.show(q.fxName_open,q.fxSettings_open,q.fxSpeed_open,function(){fa(d,false);u.isClosed||k()})}else{l.show();k()}}},fa=function(d,h){var i=B[d];if(h){i.css({zIndex:t.zIndex.animation});if(d=="south")i.css({top:z.top+z.innerHeight-i.outerHeight()});
else d=="east"&&i.css({left:z.left+z.innerWidth-i.outerWidth()})}else{w[d].isSliding||i.css({zIndex:t.zIndex.pane_normal});if(d=="south")i.css({top:"auto"});else d=="east"&&i.css({left:"auto"})}},ha=function(d,h){var i=p[d],k=O[d],l=i.slideTrigger_open;if(k&&i.slidable){if(l!="click"&&l!="dblclick"&&l!="mouseover")l="click";k[h?"bind":"unbind"](l,xa).css("cursor",h?i.sliderCursor:"default").attr("title",h?i.sliderTip:"")}},ka=function(d,h){function i(s){da(d,"closeSlider");s.stopPropagation()}var k=
p[d].slideTrigger_close,l=B[d],n=O[d];w[d].isSliding=h;da(d,"closeSlider");l.css({zIndex:h?t.zIndex.sliding:t.zIndex.pane_normal});n.css({zIndex:h?t.zIndex.sliding:t.zIndex.resizer_normal});if(k!="click"&&k!="mouseout")k="mouseout";if(h){l.bind(k,pa);n.bind(k,pa);k="mouseout";l.bind("mouseover",i);n.bind("mouseover",i)}else{l.unbind(k);n.unbind(k);k="mouseout";l.unbind("mouseover");n.unbind("mouseover");da(d,"closeSlider")}},xa=function(){var d=b(this).attr("resizer");if(w[d].isClosed){ka(d,true);
V(d,true)}},pa=function(){function d(){ka(i,false);k.isClosed||T(i)}var h=b(this),i=h.attr("pane")||h.attr("resizer");h=p[i];var k=w[i];k.isClosed||k.isResizing||(h.slideTrigger_close=="click"?d():ra(i,"closeSlider",d,300))},ia=function(d,h){var i=t[d].edge,k=t[d].dir,l=p[d],n=w[d],s=B[d],q=O[d];ca(d);n.minSize=C(n.minSize,l.minSize);if(l.maxSize>0)n.maxSize=Math.min(n.maxSize,l.maxSize);h=C(h,n.minSize);h=Math.min(h,n.maxSize);n.size=h;q.css(i,h+z[i]);s.css(t[d].sizeType,C(1,t[d].dir=="horz"?N(d,
h):R(d,h)));n.isSliding||W(k=="horz"?"all":"center");Z();aa(d);L(d,l.onresize_end||l.onresize)},W=function(d,h,i){if(!d||d=="all")d="east,west,center";var k=ma();h&&b.extend(k,h);b.each(d.split(","),function(){if(B[this]){var l=I(this),n=p[l],s=w[l],q=B[l],u=O[l],r=true,v={};if(l=="center"){k=ma();v=b.extend({},k);v.width=C(1,R(l,v.width));v.height=C(1,N(l,v.height));r=v.width>1&&v.height>1;if(b.browser.msie&&(!b.boxModel||b.browser.version<7)){B.north&&B.north.css({width:R(B.north,z.innerWidth)});
B.south&&B.south.css({width:R(B.south,z.innerWidth)})}}else{v.top=k.top;v.bottom=k.bottom;v.height=C(1,N(l,k.height));r=v.height>1}if(r){q.css(v);if(s.noRoom){s.noRoom=false;if(s.isHidden)return;else ea(l,!s.isClosed)}if(!i){aa(l);L(l,n.onresize_end||n.onresize)}}else if(!s.noRoom){s.noRoom=true;if(!s.isHidden)if(i){q.hide();u&&u.hide()}else ga(l)}}})},aa=function(d){if(!d||d=="all")d=t.allPanes;b.each(d.split(","),function(){if(X[this]){var h=I(this),i=p[h].contentIgnoreSelector,k=B[h];h=X[h];var l=
h[0],n=N(k);k.children().each(function(){if(this!=l){var s=b(this);if(!i||!s.is(i))n-=s.outerHeight()}});if(n>0)n=N(h,n);n<1?h.hide():h.css({height:n}).show()}})},Z=function(d,h){if(!d||d=="all")d=t.borderPanes;b.each(d.split(","),function(){var i=I(this),k=p[i],l=w[i],n=B[i],s=O[i],q=Y[i];if(!(!n||!s||!k.resizable&&!k.closable)){var u=t[i].dir,r=l.isClosed?"_closed":"_open";i=k["spacing"+r];var v=k["togglerAlign"+r];r=k["togglerLength"+r];var A;if(i==0)s.hide();else{!l.noRoom&&!l.isHidden&&s.show();
if(u=="horz"){A=n.outerWidth();s.css({width:C(1,R(s,A)),height:C(1,N(s,i)),left:Q(n,"left")})}else{A=n.outerHeight();s.css({height:C(1,N(s,A)),width:C(1,R(s,i)),top:z.top+S("north",true)})}if(q){if(r==0||l.isSliding&&k.hideTogglerOnSlide){q.hide();return}else q.show();if(!(r>0)||r=="100%"||r>A){r=A;n=0}else if(typeof v=="string")switch(v){case "top":case "left":n=0;break;case "bottom":case "right":n=A-r;break;default:n=Math.floor((A-r)/2)}else{n=parseInt(v);n=v>=0?n:A-r+n}A=k.togglerContent_open?
q.children(".content-open"):false;var G=k.togglerContent_closed?q.children(".content-closed"):false;v=l.isClosed?G:A;if(A)A.css("display",l.isClosed?"none":"block");if(G)G.css("display",l.isClosed?"block":"none");if(u=="horz"){l=R(q,r);q.css({width:C(0,l),height:C(1,N(q,i)),left:n});v&&v.css("marginLeft",Math.floor((l-v.outerWidth())/2))}else{l=N(q,r);q.css({height:C(0,l),width:C(1,R(q,i)),top:n});v&&v.css("marginTop",Math.floor((l-v.outerHeight())/2))}}if(h&&k.initHidden){s.hide();q&&q.hide()}}}})},
qa=function(){var d=z.innerWidth,h=z.innerHeight;z=w.container=na(M);var i=z.innerHeight<h,k=z.innerWidth<d,l,n;if(i||k)b.each(["south","north","east","west"],function(s,q){l=w[q];n=t[q].dir;if(!l.isClosed&&(i&&n=="horz"||k&&n=="vert")){ca(q);l.size>l.maxSize&&ia(q,l.maxSize)}});W("all");Z("all")},M=b(this).css({overflow:"hidden"}),B={},X={},O={},Y={},t={allPanes:"north,south,east,west,center",borderPanes:"north,south,east,west",zIndex:{resizer_normal:1,pane_normal:2,mask:4,sliding:100,resizing:1E4,
