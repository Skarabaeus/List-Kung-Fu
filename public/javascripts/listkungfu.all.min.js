...
c,i){var e=this,f=$($.mustache(c,b.list_item));f.data("data",b);f.data("isFullsize",false);i?e.listItemList.prepend(f):e.listItemList.append(f);f.find(".handle").height(f.height());f.bind("keyup","space",function(){e.toolbar.find("#list-item-completed").effect("puff",{},300,function(){$(this).show();f.find(".undo").length===0?e.toolbar.find("#list-item-completed").trigger("click"):f.find(".undo").trigger("click")});return false});f.bind("keydown","del",function(){e.toolbar.find("#list-item-delete").effect("puff",
{},300,function(){$(this).show();e.toolbar.find("#list-item-delete").trigger("click")});return false});f.bind("focus",function(j){if($(j.target).hasClass("row")){e.listItemList.find(".row").removeClass("selected-row");$(j.target).addClass("selected-row");$(j.target).data("data");if(e.toolbar)$(j.target).data("isFullsize")===true?e.toolbar.find("#list-item-fullsize").button("option","icons",{primary:"ui-icon-zoomout"}):e.toolbar.find("#list-item-fullsize").button("option","icons",{primary:"ui-icon-zoomin"});
e._SetSelectedListItem($(j.target),$(j.target).data("data"))}});$.browser.msie&&f.bind("click ",function(){f.focus();return false});f.bind("keydown","down",function(j){j.preventDefault();$(j.target).nextAll("div:visible").first().focus();return false});f.bind("keydown","up",function(j){j.preventDefault();$(j.target).prevAll("div:visible").first().focus();return false});f.bind("keydown dblclick","return",function(){if(f.find("form").length>0)return false;e.toolbar.find("#list-item-edit").effect("puff",
{},300,function(){$(this).show()});e.listItemList.find(".row").find("form").remove();ListListItem.Edit({successCallback:function(j){if($.browser.msie&&f.find("form").length>0)return false;var m=$(j),s=f.data("isFullsize");e.listItemList.find(".row").hide();e.selectedListItem.element.show();m.hide();e.selectedListItem.element.prepend(m);m.find("textarea").markItUp(mySettings);e._SetupDeadlineButton(m);e._SetupCustomDeadlinePicker();s===false&&e._ToggleFullsize(f);e.selectedListItem.element.find(".handle").hide();
m.show("slow",function(){m.find("textarea").focus()});m.find("textarea").bind("keydown","esc",function(){m.find("#cancel-edit").trigger("click")});m.find(".deadline-button").bind("keydown click","return",function(u){u.preventDefault();var p=f.find("form").serializeForm();p.list_item.deadlineType=$(u.target).attr("data-deadline");p.list_item.customDeadlineValue=$("#custom-deadline-value").val();ListListItem.Update({send:p,successCallback:function(E,y){m.hide("slow",function(){$(this).remove();e.selectedListItem.element.find(".handle").show();
f.find(".list-item-content").html(y.list_item.body_rendered);f.find(".list-item-deadline").html(y.list_item.deadline_in_words);f.find(".handle").height(f.find(".list-item-content").height());s===false&&e._ToggleFullsize(f);e._SetSelectedListItem(f,y);f.data("data",y);e.listItemList.find(".row").show();f.focus()})},lists:e.selectedListItem.data.list_item.list_id,list_items:e.selectedListItem.data.list_item.id});return false});m.find("#cancel-edit").bind("keydown click","return",function(u){u.preventDefault();
e.selectedListItem.element.find(".handle").show();m.hide("slow",function(){$(this).remove();s===false&&e._ToggleFullsize(f);e.listItemList.find(".row").show();f.focus()});return false});return false},lists:e.selectedListItem.data.list_item.list_id,list_items:e.selectedListItem.data.list_item.id})});f.bind("keydown","left",function(){e._trigger("SelectLastList",0,{})});f.bind("keydown","shift+return",function(){e.toolbar.find("#list-item-new").effect("puff",{},300,function(){$(this).show();e._AddNewListItem()})});
f.bind("keydown","l",function(){e.toolbar.find("#list-item-fullsize").effect("puff",{},300,function(){e.toolbar.find("#list-item-fullsize").trigger("click");$(this).show()});return false});ListKungFu&&ListKungFu.LayoutCenter&&f.draggable({helper:function(){return $('<div class="list-item-drag-helper">'+f.find(".list-item-content").text().substring(0,20)+" . . .</div>").get(0)},appendTo:ListKungFu.LayoutCenter,revert:"invalid",handle:".handle",cursorAt:{top:10,left:10},iframeFix:true,start:function(){document.onselectstart=
