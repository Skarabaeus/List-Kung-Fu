...
var Controller=function(h){var b={},u=function(k){var l=$(k).length>0?{}:false;$(k).each(function(f,m){var p=$(m).attr("model"),r=$(m).attr("field"),n=$(m).text();l[p]=l[p]||{};l[p][r]=l[p][r]||[];l[p][r].push(n)});return l};b.ClearFlash=function(){b.flash={notice:"",error:"",warning:"",message:""}};b.ClearFlash();b.SetFlash=function(k){b.flash.notice=k.getResponseHeader("X-Flash-Notice")||"";b.flash.error=k.getResponseHeader("X-Flash-Error")||"";b.flash.warning=k.getResponseHeader("X-Flash-Warning")||
"";b.flash.message=k.getResponseHeader("X-Flash-Message")||"";if(typeof h.onFlashUpdate==="function")if(b.flash.notice!==""||b.flash.error!==""||b.flash.warning!==""||b.flash.message!=="")h.onFlashUpdate(b.flash.notice,b.flash.error,b.flash.warning,b.flash.message)};b.baseURL=h.base_url||document.location.protocol+"//"+document.location.hostname+(document.location.port!==""?":"+document.location.port:"")+"/";b.route=h.route;b.DefaultCallback=function(k,l,f,m){b.SetFlash(m);var p=$.parseJSON($(l).find("JSON").text()),
r=$(l).find("Template").text();l=u($(l).find("Error"));typeof k==="function"&&k(r,p,f,m,l);b.ClearFlash()};b.DefaultErrorCallback=function(){$('<div id="dialog-confirm"> \t\t\t<span class="ui-icon ui-icon-alert" style="float:left; margin:0 7px 20px 0;"> \t\t\t</span>Something went wrong :-( Either try again or reload the page!</div>').dialog({resizable:false,modal:true,buttons:{"Try again":function(){$(this).dialog("close")},Reload:function(){window.location.reload()}}})};b.ConstructRoute=function(k){for(var l=
"",f="",m=0;m<b.route.length;m++){l+=b.route[m];f=k[b.route[m]]||"";if(f!=="")l+="/"+f;if(m!==b.route.length-1)l+="/"}return l};b.Index=function(k){var l=k.successCallback||null,f=b.ConstructRoute(k);$.ajax({url:b.baseURL+f,dataType:"xml",type:"GET",processData:true,contentType:"application/json",data:k.send,success:function(m,p,r){b.DefaultCallback(l,m,p,r)},error:function(m,p,r){b.DefaultErrorCallback(m,p,r)}})};b.Show=function(k){var l=k.successCallback||null;k=b.ConstructRoute(k);$.ajax({url:b.baseURL+
k,dataType:"xml",type:"GET",processData:false,contentType:"application/json",success:function(f,m,p){b.DefaultCallback(l,f,m,p)},error:function(f,m,p){b.DefaultErrorCallback(f,m,p)}})};b.New=function(k){var l=k.successCallback||null;k=b.ConstructRoute(k);$.ajax({url:b.baseURL+k+"/new",dataType:"xml",type:"GET",processData:false,contentType:"application/json",success:function(f,m,p){b.DefaultCallback(l,f,m,p)},error:function(f,m,p){b.DefaultErrorCallback(f,m,p)}})};b.Create=function(k){var l=k.successCallback||
null,f=b.ConstructRoute(k);k=JSON.stringify(k.send);$.ajax({url:b.baseURL+f,dataType:"xml",type:"POST",processData:false,contentType:"application/json",data:k,success:function(m,p,r){b.DefaultCallback(l,m,p,r)},error:function(m,p,r){b.DefaultErrorCallback(m,p,r)}})};b.Edit=function(k){var l=k.successCallback||null;k=b.ConstructRoute(k);$.ajax({url:b.baseURL+k+"/edit",dataType:"xml",type:"GET",parseData:false,contentType:"application/json",success:function(f,m,p){b.DefaultCallback(l,f,m,p)},error:function(f,
m,p){b.DefaultErrorCallback(f,m,p)}})};b.Update=function(k){var l=k.successCallback||null,f=b.ConstructRoute(k);k=JSON.stringify(k.send);k=k.replace(/\\u000a/g,"\\n");$.ajax({url:b.baseURL+f,dataType:"xml",type:"POST",processData:false,contentType:"application/json",data:k,beforeSend:function(m){m.setRequestHeader("X-Http-Method-Override","PUT")},success:function(m,p,r){b.DefaultCallback(l,m,p,r)},error:function(m,p,r){b.DefaultErrorCallback(m,p,r)}})};b.Destroy=function(k){var l=k.successCallback||
null;k=b.ConstructRoute(k);$.ajax({url:b.baseURL+k,dataType:"xml",type:"POST",processData:false,contentType:"application/json",beforeSend:function(f){f.setRequestHeader("X-Http-Method-Override","DELETE")},success:function(f,m,p){b.DefaultCallback(l,f,m,p)},error:function(f,m,p){b.DefaultErrorCallback(f,m,p)}})};return b},List=Controller({route:["lists"],onFlashUpdate:function(h,b){ListKungFu.LayoutSouth.StatusBar("SetAlert",b);ListKungFu.LayoutSouth.StatusBar("SetNotice",h)}}),ListListItem=Controller({route:["lists",
...
b+'" type="hidden" />';m.hide().append(f).appendTo("body");l.preventDefault();m.submit()});var k=function(){h(this).find("input[data-disable-with]").each(function(){var l=h(this);l.data("enable-with",l.val()).attr("value",l.attr("data-disable-with")).attr("disabled","disabled")})};h("form[data-remote]:has(input[data-disable-with])").live("ajax:before.rails",k);h("form:not([data-remote]):has(input[data-disable-with])").live("submit.rails",k);h("form[data-remote]:has(input[data-disable-with])").live("ajax:complete.rails",
function(){h(this).find("input[data-disable-with]").each(function(){var l=h(this);l.removeAttr("disabled").val(l.data("enable-with"))})});k=h().jquery;if(k==="1.4"||k==="1.4.1"||k==="1.4.2")alert("This rails.js does not support the jQuery version you are using. Please read documentation.")});
(function(h){h.confirmationDialog=function(b,u,k,l){dialog=h('<div id="dialog-confirm"> \t\t\t<span class="ui-icon ui-icon-alert" style="float:left; margin:0 7px 20px 0;"> \t\t\t</span>'+l+"</div>").dialog({resizable:false,modal:true,autoOpen:false,width:400,open:function(){dialog.parent().find(".ui-dialog-buttonpane > button").first().focus()}});l={};l[b]=function(){h(dialog).dialog("close");u()};l[k]=function(){h(dialog).dialog("close")};dialog.dialog("option","buttons",l);dialog.parent().find(".ui-dialog-buttonpane > button").bind("keydown",
...
f.parent();f.hide("slow",function(){f.remove();E.find(".dashboard-item").length===0&&E.append('<p class="nothing-todo">no items scheduled</p>')});l(z,t,{highlightCategory:true,addType:"prepend"})},lists:r.list_item.list_id,list_items:r.list_item.id,send:r});m.datepicker("hide")}})},k=function(f,m,p){p=p||{};f.find(".nothing-todo").remove();p.addType==="prepend"?f.find("div[data-type=scheduleColumn]").first().prepend(m):f.find("div[data-type=scheduleColumn]").first().append(m);p.highlightCategory===
true&&m.effect("highlight",{},2E3)},l=function(f,m,p){var r=$($.mustache(m,f.list_item));switch(f.list_item.deadline_category){case "today":k(b.today,r,p);break;case "tomorrow":k(b.tomorrow,r,p);break;case "this week":k(b.thisweek,r,p);break;case "next week":k(b.nextweek,r,p);break;default:k(b.later,r,p)}r.data("data",f);r.find(".dashboard-list-item-title").bind("click",function(){b._trigger("OpenList",0,{selectedList:f.list_item})});r.find(".dashboard-item-more").bind("click",function(){b._trigger("OpenList",
...
(function(h){function b(u){if(typeof u.data==="string"){var k=u.handler,l=u.data.toLowerCase().split(" ");u.handler=function(f){if(!(this!==f.target&&(/textarea|select/i.test(f.target.nodeName)||f.target.type==="text"))){var m=f.type!=="keypress"&&h.hotkeys.specialKeys[f.which],p=String.fromCharCode(f.which).toLowerCase(),r="",n={};if(f.altKey&&m!=="alt")r+="alt+";if(f.ctrlKey&&m!=="ctrl")r+="ctrl+";if(f.metaKey&&!f.ctrlKey&&m!=="meta")r+="meta+";if(f.shiftKey&&m!=="shift")r+="shift+";if(m)n[r+m]=
true;else{n[r+p]=true;n[r+h.hotkeys.shiftNums[p]]=true;if(r==="shift+")n[h.hotkeys.shiftNums[p]]=true}m=0;for(p=l.length;m<p;m++)if(n[l[m]])return k.apply(this,arguments)}}}}h.hotkeys={version:"0.8",specialKeys:{8:"backspace",9:"tab",13:"return",16:"shift",17:"ctrl",18:"alt",19:"pause",20:"capslock",27:"esc",32:"space",33:"pageup",34:"pagedown",35:"end",36:"home",37:"left",38:"up",39:"right",40:"down",45:"insert",46:"del",96:"0",97:"1",98:"2",99:"3",100:"4",101:"5",102:"6",103:"7",104:"8",105:"9",
...
(function(){var h=function(){var b=null,u=function(){b._trigger("ContentDimensionsChanged",0,{})},k=function(t){var z=t.find("#deadline-settings-wrapper"),E=t.find("#save-deadline");E.bind("click keydown","return",function(){E.data("settings-visible")?z.hide("fast",function(){z.find(".deadline-button").first();E.data("settings-visible",false)}):z.show("fast",function(){z.find(".deadline-button").first().focus();E.data("settings-visible",true)});return false})},l=function(){$("#custom-deadline-display").datepicker({minDate:new Date,
altField:"#custom-deadline-value",altFormat:"yy,mm,dd",dateFormat:"dd.mm.yy"});$("#custom-deadline-value").val($.datepicker.formatDate("yy,mm,dd",new Date));$("#custom-deadline-display").val($.datepicker.formatDate("dd.mm.yy",new Date))},f=function(t,z,E){var j=$($.mustache(z,t.list_item));j.data("data",t);j.data("isFullsize",false);E?b.listItemList.prepend(j):b.listItemList.append(j);j.find(".handle").height(j.height());j.bind("keydown","space",function(){b.toolbar.find("#list-item-completed").effect("puff",
{},300,function(){$(this).show();j.find(".undo").length===0?b.toolbar.find("#list-item-completed").trigger("click"):j.find(".undo").trigger("click")})});j.bind("keydown","del",function(){b.toolbar.find("#list-item-delete").effect("puff",{},300,function(){$(this).show();b.toolbar.find("#list-item-delete").trigger("click")})});j.bind("focus",function(w){if($(w.target).hasClass("row")){b.listItemList.find(".row").removeClass("selected-row");$(w.target).addClass("selected-row");$(w.target).data("data");
if(b.toolbar)$(w.target).data("isFullsize")===true?b.toolbar.find("#list-item-fullsize").button("option","icons",{primary:"ui-icon-zoomout"}):b.toolbar.find("#list-item-fullsize").button("option","icons",{primary:"ui-icon-zoomin"});var v=$(w.target);w=$(w.target).data("data");b.selectedListItem={element:v,data:w}}});$.browser.msie&&j.bind("click ",function(){j.focus();return false});j.bind("keydown","down",function(w){w.preventDefault();$(w.target).nextAll("div:visible").first().focus();return false});
j.bind("keydown","up",function(w){w.preventDefault();$(w.target).prevAll("div:visible").first().focus();return false});j.bind("keydown dblclick","return",function(){if(j.find("form").length>0)return false;b.toolbar.find("#list-item-edit").effect("puff",{},300,function(){$(this).show()});b.listItemList.find(".row").find("form").hide("slow",function(){$(this).remove()});ListListItem.Edit({successCallback:function(w){var v=$(w),F=j.data("isFullsize");b.listItemList.find(".row").hide();b.selectedListItem.element.show();
v.hide();b.selectedListItem.element.prepend(v);v.find("textarea").markItUp(mySettings);k(v);l();F===false&&m(j);b.selectedListItem.element.find(".handle").hide();v.show("slow",function(){v.find("textarea").focus()});v.find("textarea").bind("keydown","esc",function(){v.find("#cancel-edit").trigger("click")});v.find(".deadline-button").bind("keydown click","return",function(G){G.preventDefault();var M=j.find("form").serializeForm();M.list_item.deadlineType=$(G.target).attr("data-deadline");M.list_item.customDeadlineValue=
$("#custom-deadline-value").val();ListListItem.Update({send:M,successCallback:function(O,P){v.hide("slow",function(){$(this).remove();b.selectedListItem.element.find(".handle").show();j.find(".list-item-content").html(P.list_item.body_rendered);j.find(".list-item-deadline").html(P.list_item.deadline_in_words);j.find(".handle").height(j.find(".list-item-content").height());F===false&&m(j);b.selectedListItem={element:j,data:P};j.data("data",P);b.listItemList.find(".row").show();j.focus()})},lists:b.selectedListItem.data.list_item.list_id,
list_items:b.selectedListItem.data.list_item.id});return false});v.find("#cancel-edit").bind("keydown click","return",function(G){G.preventDefault();b.selectedListItem.element.find(".handle").show();v.hide("slow",function(){$(this).remove();F===false&&m(j);b.listItemList.find(".row").show();j.focus()});return false});return false},lists:b.selectedListItem.data.list_item.list_id,list_items:b.selectedListItem.data.list_item.id})});j.bind("keydown","left",function(){b._trigger("SelectLastList",0,{})});
j.bind("keydown","shift+return",function(){b.toolbar.find("#list-item-new").effect("puff",{},300,function(){$(this).show();N()})});j.bind("keydown","l",function(){b.toolbar.find("#list-item-fullsize").effect("puff",{},300,function(){b.toolbar.find("#list-item-fullsize").trigger("click");$(this).show()});return false});ListKungFu&&ListKungFu.LayoutCenter&&j.draggable({helper:function(){return $('<div class="list-item-drag-helper">'+j.find(".list-item-content").text().substring(0,20)+" . . .</div>").get(0)},
appendTo:ListKungFu.LayoutCenter,revert:"invalid",handle:".handle",cursorAt:{top:10,left:10},iframeFix:true,start:function(){document.onselectstart=function(){return false};b._trigger("StartedDraggingListItem",0,{dragType:"listitem"})},stop:function(){document.onselectstart=null}});p(j)},m=function(t){if(t.data("isFullsize")===true){p(t,false);t.data("isFullsize",false);b.toolbar.find("#list-item-fullsize").button("option","icons",{primary:"ui-icon-zoomin"})}else{p(t,true);t.data("isFullsize",true);
b.toolbar.find("#list-item-fullsize").button("option","icons",{primary:"ui-icon-zoomout"})}},p=function(t,z){t.height()>150&&!z?t.height(150):t.height("auto");u()},r=function(t,z){ListListItem.Destroy({successCallback:function(){t.hide("slow",function(){var E=t.next(".row");E.length>0||(E=t.prev(".row"));E.focus();t.remove();b.toolbar.find("#list-item-search").trigger("ClearValue")})},lists:z.list_item.list_id,list_items:z.list_item.id})},n=function(t,z){z.list_item.completed=true;ListListItem.Update({successCallback:function(){var E=
"list-item-undo-"+z.list_item.id;t.find(".list-item-wrapper").hide("slow",function(){var j=$('<div class="undo clickable"><img src="/images/undo-icon.gif"/>Undo</div>');j.bind("click",function(){I(t,z,E)});t.append(j);p(t);j.delay(5E3,E).queue(E,function(){t.hide("slow",function(){t.remove()})}).dequeue(E);b.toolbar&&b.toolbar.find("#showCompleted").get(0).checked===true&&B(true);t.focus()})},lists:z.list_item.list_id,list_items:z.list_item.id,send:z})},I=function(t,z,E){E!==""&&t.clearQueue(E);z.list_item.completed=
false;ListListItem.Update({successCallback:function(){t.find(".undo").hide("slow",function(){$(this).remove()});t.find(".list-item-wrapper").show("slow",function(){p(t)});b.toolbar&&b.toolbar.find("#showCompleted").get(0).checked===true&&B(true)},lists:z.list_item.list_id,list_items:z.list_item.id,send:z})},B=function(t){if(t===true){if(b.completedList){b.completedList.remove();b.completedList=null}b.completedList=$('<div id="completedList"></div>');t=b.element.data("data-list");ListListItem.Index({successCallback:function(z,
E){$.each(E,function(j,w){b.completedList.append("<div>"+w.list_item.body_rendered+"</div>")});E.length===0&&b.completedList.append("<div>No completed items</div>");b.wrapper.prepend(b.completedList)},send:{show:"completed"},lists:t.list.id})}else{b.completedList.remove();b.completedList=null}},L=function(){b.header.html("");b.toolbar=$('<div id="list-item-toolbar"> \t\t\t\t<div id="list-name"></div><div id="list-item-toolbar-buttons"> \t\t\t\t<button id="list-item-completed">Completed [space]</button> \t\t\t\t<button id="list-item-new">Create [shift+return]</button> \t\t\t\t<button id="list-item-delete">Delete [del]</button> \t\t\t\t<button id="list-item-edit">Edit [return]</button> \t\t\t\t<button id="list-item-fullsize">Fullsize [l]</button> \t\t\t\t<input type="input" id="list-item-search"/> \t\t\t\t<input type="checkbox" id="showCompleted"/> \t\t\t\t<label for="showCompleted">Show Completed Items</label> \t\t\t\t<div id="list-item-search-cancel">&nbsp;</div> \t\t\t\t</div></div>');
...
function(){b.toolbar.find("#list-item-search").val("").trigger("keyup")});b.toolbar.find("#list-item-search").bind("ClearValue",function(t){$(t.target).val("");b.listItemList.find(".row").show()});b.header.append(b.toolbar)},N=function(){var t=b.element.data("data-list");ListListItem.New({successCallback:function(z){var E=$(z);b.listItemList.prepend(E);k(E);l();E.find("textarea").markItUp(mySettings);E.find("textarea").focus();b.listItemList.find(".row").hide();E.find("#cancel-edit").bind("click",
function(j){j.preventDefault();E.hide("slow",function(){$(this).remove();b.listItemList.find(".row").show();b.listItemList.find(".row").first().focus()})});E.find(".deadline-button").bind("click",function(j){j.preventDefault();var w=E.serializeForm();w.list_item.body=w.list_item.body.replace(/\n/g,"\n\n");w.list_item.deadlineType=$(j.target).attr("data-deadline");w.list_item.customDeadlineValue=$("#custom-deadline-value").val();ListListItem.Create({send:w,successCallback:function(v,F){E.hide("slow",
...
destroy:function(){b.children().remove();$(document).unbind("keydown","c");$(document).unbind("keyup","f")},RemoveList:function(){b.wrapper.remove();b.header.remove();$(document).unbind("keydown","c");$(document).unbind("keyup","f")},OpenList:function(t){b.RemoveList();b._create();b.element.data("data-list",t);ListListItem.Index({successCallback:function(z,E){L();$.each(E,function(j,w){f(w,z)});t.id?b.listItemList.find(".row").each(function(){var j=$(this);if(j.data("data").list_item.id===t.id){j.focus();
...
(function(){var h=function(){var b=null,u={activeClass:"ui-state-default",hoverClass:"ui-state-hover",accept:".tag",drop:function(j,w){if($(this).position().top<b.wrapper.height()){var v=$(this),F=w.draggable.data("data"),G=v.data("data");G.list.tag_id=F.tag.id;List.Update({send:G,successCallback:function(M,O){var P=r(O,M);v.replaceWith(P);I();n();P.focus()},lists:G.list.id})}}},k=function(j){return{activeClass:"ui-state-default",hoverClass:"ui-state-hover",accept:".list-item",drop:function(w,v){if($(this).position().top<
b.wrapper.height()){var F=v.draggable.data("data"),G=$(this).data("data");F={list_item:{body:F.list_item.body.replace(/\n/g,"\n\n")}};ListListItem.Create({send:F,successCallback:function(){$(w.target).find(".fake-drop").remove();j.effect("highlight",{},3E3)},lists:G.list.id})}},over:function(w,v){var F=$('<div class="fake-drop"></div>');F.html("<p>"+v.helper.text()+"</p>");$(w.target).append(F)},out:function(w){$(w.target).find(".fake-drop").remove()}}},l=function(j,w){for(var v in w){var F=w[v],
G;for(G in F){for(var M=F[G],O=$('<div class="error_explanation"><ul></ul></div>'),P=O.find("ul"),V=j.find("*[name="+(v+"["+G+"]")+"]"),W=0;W<M.length;W++){V.parent(".field").addClass("field_with_errors");P.append("<li>"+M[W]+"</li>")}V.parent(".field").append(O)}}},f=function(j){j.find(".field").removeClass("field_with_errors");j.find(".error_explanation").remove()},m=function(j){var w=b.selectedList.element.next(),v=b.selectedList.element.prev();j=j||0;List.Destroy({successCallback:function(){b.selectedList.element.delay(j).fadeOut(function(){$(this).remove();
w.length>0?w.focus():v.focus()});b._trigger("CloseList",0,{})},lists:b.selectedList.data.list.id})},p=function(){var j=$('<div id="list-toolbar"> \t\t\t\t<button id="list-new">Create [shift+return]</button> \t\t\t\t<button id="list-delete">Delete [del]</button> \t\t\t\t<button id="list-edit">Edit [space]</button> \t\t\t\t<input type="text" id="search-list" /> \t\t\t\t<div id="list-search-cancel">&nbsp;</div> \t\t\t\t</div>');b.element.find("div.header").append(j);j.find("#list-new").button({text:false,
icons:{primary:"ui-icon-plusthick"}});j.find("#list-delete").button({text:false,icons:{primary:"ui-icon-trash"}});j.find("#list-edit").button({text:false,icons:{primary:"ui-icon-pencil"}});j.find("#list-delete").bind("click",function(w){if(b.selectedList!==null){if(typeof b.deleteDialog==="undefined")b.deleteDialog=$.confirmationDialog("Delete List",function(){if(b.listForm!==null){b.listForm.bind("FormHidden",function(){m(1E3)});z()}else m()},"Cancel","Do you really want to delete?");b.deleteDialog.dialog("open")}else $("#notice").text("Select the list which you want to delete").fadeIn().delay(5E3).fadeOut();
$(w.target).blur();return false});j.find("#list-new").bind("click",function(w){b.listlist.hide("slide",{direction:"left"},"slow",function(){if(b.listForm!=null){b.listForm.remove();b.listForm=null}b.listForm=$('<div class="ui-layout-content" id="list-form"></div>');b.wrapper.append(b.listForm);List.New({successCallback:function(v,F){b.listForm.append(v);b.listForm.show("slide",{direction:"left"},"slow",function(){b.listForm.find("#list_title").first().focus()});b._trigger("CloseList",0,{});b.listForm.find("#list-back-button").bind("click",
function(){z()});b.listForm.find("#list_title").bind("keydown","esc",function(){z()});b.listForm.bind("submit",function(G){G.preventDefault();G=$(this).find("form").serializeForm();F.list=G.list;List.Create({send:F,successCallback:function(M,O,P,V,W){f(b.listForm);if(W===false){z(O,M);b.toolbar.find("#search-list").trigger("ClearValue")}else l(b.listForm,W)}});return false});b.selectedList=null}})});$(w.target).blur();return false});j.find("#search-list").bind("keyup",function(){var w=$(this).val();
if(w==="")b.listlist.find(".row").show();else{b.listlist.find(".row:not(:Contains('"+w+"'))").hide();b.listlist.find(".row:Contains('"+w+"')").show()}I();n()});j.find("#list-search-cancel").bind("click",function(){j.find("#search-list").val("").trigger("keyup")});j.find("#search-list").bind("ClearValue",function(w){$(w.target).val("");b.listlist.find(".row").show()});j.find("#list-edit").bind("click",function(){b.listlist.hide("slide",{direction:"left"},"slow",function(){if(b.listForm!=null){b.listForm.remove();
b.listForm=null}b.listForm=$('<div class="ui-layout-content" id="list-form"></div>');b.wrapper.append(b.listForm);List.Edit({successCallback:function(w){b.listForm.append(w);b.listForm.show("slide",{direction:"left"},"slow",function(){b.listForm.find("#list_title").first().focus()});b.listForm.find("#list-back-button").bind("click",function(){z()});b.listForm.find("#list_title").bind("keydown","esc",function(){z()});b.listForm.bind("submit",function(v){v.preventDefault();v=$(this).find("form").serializeForm();
List.Update({send:v,successCallback:function(F,G,M,O,P){f(b.listForm);P===false?z(G,F):l(b.listForm,P)},lists:b.selectedList.data.list.id});return false})},lists:b.selectedList.data.list.id})})});return j},r=function(j,w){var v=$($.mustache(w,j.list));v.attr("tabindex",0);v.data("data",j);v.addClass("row");$.browser.msie&&v.bind("click",function(){v.focus();return false});v.bind("keydown","down",function(F){F.preventDefault();$(F.target).nextAll("div:visible").first().focus();return false});v.bind("keydown",
"up",function(F){F.preventDefault();$(F.target).prevAll("div:visible").first().focus();return false});v.bind("focus",function(F){F=$(F.target);b.selectedList={element:F,data:F.data("data")};b.listlist.find(".row").removeClass("selected-row");F.addClass("selected-row");b.listlist.find(".assigned-tags").hide();F.find(".assigned-tags").show();I()});v.bind("keydown dblclick","return",function(){b._trigger("OpenList",0,{selectedList:v.data("data")})});v.bind("keydown","right",function(){b._trigger("SelectListItem",
0,{})});v.bind("keydown","shift+return",function(){b.toolbar.find("#list-new").effect("puff",{},300,function(){$(this).show();$(this).trigger("click")});return false});v.bind("keydown","del",function(){b.toolbar.find("#list-delete").effect("puff",{},300,function(){$(this).show()}).trigger("click")});v.bind("keydown","space",function(){b.toolbar.find("#list-edit").effect("puff",{},300,function(){$(this).show();$(this).trigger("click")});return false});v.droppable(k(v));v.find(".assigned-tag-delete").bind("click",
function(F){var G=$(F.target).parent(".assigned-tag");F=G.attr("data-id");var M=v.data("data");M.list.tag_id=F;M.list.tag_action="remove";List.Update({send:M,successCallback:function(O,P){G.hide("slow",function(){G.remove();var V=r(P,O);v.replaceWith(V);I();n();V.focus();E()})},lists:M.list.id})});v.find(".assigned-tags").hide();return v},n=function(){b._trigger("ContentDimensionsChanged",0,{})},I=function(){b.listlist.find(".row").each(function(){var j=$(this),w=j.find(".list-tag");j=j.find(".list-name");
w.height(j.height()+10+"px")})},B=function(j,w){b.listlist=$('<div id="list-list"></div>');b.wrapper.append(b.listlist);for(var v=0;v<j.length;v++)b.listlist.append(r(j[v],w));I();n()},L=function(){b.selectedList?b.selectedList.element.effect("highlight",{},300,function(){$(this).show();$(this).focus()}):b.listlist.find(".row").first().effect("highlight",{},300,function(){$(this).show();$(this).focus()})},N=function(){$(document).bind("keydown","ctrl+l",function(){L()});$(document).bind("keydown",
"ctrl+f",function(){b.toolbar.find("#search-list").focus()});$(document).bind("keydown","shift+f",function(){b.toolbar.find("#search-list").focus();return false})},t=function(j,w){b.listlist.show("slide",{direction:"left"},"slow",function(){if(j){var v=r(j,w);b.selectedList===null||b.selectedList.data.list.id!==j.list.id?b.listlist.prepend(v):b.selectedList.element.replaceWith(v);I();b.selectedList={data:j,element:v}}b.selectedList&&b.selectedList.element.focus()})},z=function(j,w){b.listForm.hide("slide",
{direction:"left"},"slow",function(){t(j,w);b.listForm.trigger("FormHidden");b.selectedList?b.selectedList.element.focus():b.listlist.find(".row").first().focus();b.listForm.remove();b.listForm=null})},E=function(){b.toolbar.find("#search-list").trigger("keyup");b.listlist.find(".row:visible").each(function(){var j=$(this),w=j.data("data"),v=0;if(b.selectedTags.length>0){for(var F=0;F<w.list.tags.length;F++)for(var G=0;G<b.selectedTags.length;G++)b.selectedTags[G]===w.list.tags[F].id&&v++;v===b.selectedTags.length?
j.show():j.hide()}})};return{options:{},selectedList:null,_create:function(){b=this;b.wrapper=b.element.find("div#list-wrapper");b.toolbar=p(b);b.listForm=null;b.selectedTags=[];List.Index({successCallback:function(j,w){b.wrapper.find(".row").remove();B(w,j);b.listlist.find(".row").first().focus()}});N()},SetupDroppable:function(j){b.listlist.find(".row").each(function(){var w=$(this);w.droppable("destroy");switch(j){case "listitem":w.droppable(k(w));break;case "tag":w.droppable(u)}})},ReloadLists:function(){List.Index({successCallback:function(j,
w){b.wrapper.find(".row").remove();B(w,j);b.toolbar.find("#search-list").trigger("keyup");E()}})},SelectList:function(){L()},FilterByTags:function(j){b.selectedTags=j;E()},destroy:function(){b.toolbar.remove();b.listForm.remove();b.wrapper.find(".row").remove()}}}();$.widget("ui.ListView",h)})();
(function(h){h.fn.serializeForm=function(){var b=h(this).find('*[name*="]"]'),u={};h.each(b,function(k,l){var f=h(l),m=f.get(0).name.split("[")[0],p=f.get(0).name.split("[")[1];p=p.replace("]","");f=f.val();u[m]=u[m]||{};u[m][p]=f});return u}})(jQuery);
...
(function(){function h(n){return n<10?"0"+n:n}function b(n){l.lastIndex=0;return l.test(n)?'"'+n.replace(l,function(I){var B=p[I];return typeof B==="string"?B:"\\u"+("0000"+I.charCodeAt(0).toString(16)).slice(-4)})+'"':'"'+n+'"'}function u(n,I){var B,L,N,t,z=f,E,j=I[n];if(j&&typeof j==="object"&&typeof j.toJSON==="function")j=j.toJSON(n);if(typeof r==="function")j=r.call(I,n,j);switch(typeof j){case "string":return b(j);case "number":return isFinite(j)?String(j):"null";case "boolean":case "null":return String(j);
case "object":if(!j)return"null";f+=m;E=[];if(Object.prototype.toString.apply(j)==="[object Array]"){t=j.length;for(B=0;B<t;B+=1)E[B]=u(B,j)||"null";N=E.length===0?"[]":f?"[\n"+f+E.join(",\n"+f)+"\n"+z+"]":"["+E.join(",")+"]";f=z;return N}if(r&&typeof r==="object"){t=r.length;for(B=0;B<t;B+=1){L=r[B];if(typeof L==="string")if(N=u(L,j))E.push(b(L)+(f?": ":":")+N)}}else for(L in j)if(Object.hasOwnProperty.call(j,L))if(N=u(L,j))E.push(b(L)+(f?": ":":")+N);N=E.length===0?"{}":f?"{\n"+f+E.join(",\n"+f)+
...
{"":n})};if(typeof JSON.parse!=="function")JSON.parse=function(n,I){function B(N,t){var z,E,j=N[t];if(j&&typeof j==="object")for(z in j)if(Object.hasOwnProperty.call(j,z)){E=B(j,z);if(E!==undefined)j[z]=E;else delete j[z]}return I.call(N,t,j)}var L;n=String(n);k.lastIndex=0;if(k.test(n))n=n.replace(k,function(N){return"\\u"+("0000"+N.charCodeAt(0).toString(16)).slice(-4)});if(/^[\],:{}\s]*$/.test(n.replace(/\\(?:["\\\/bfnrt]|u[0-9a-fA-F]{4})/g,"@").replace(/"[^"\\\n\r]*"|true|false|null|-?\d+(?:\.\d*)?(?:[eE][+\-]?\d+)?/g,
"]").replace(/(?:^|:|,)(?:\s*\[)+/g,""))){L=eval("("+n+")");return typeof I==="function"?B({"":L},""):L}throw new SyntaxError("JSON.parse");}})();
var Mustache=function(){var h=function(){};h.prototype={otag:"{{",ctag:"}}",pragmas:{},buffer:[],pragmas_implemented:{"IMPLICIT-ITERATOR":true},context:{},render:function(b,u,k,l){if(!l){this.context=u;this.buffer=[]}if(!this.includes("",b))if(l)return b;else{this.send(b);return}b=this.render_pragmas(b);b=this.render_section(b,u,k);if(l)return this.render_tags(b,u,k,l);this.render_tags(b,u,k,l)},send:function(b){b!=""&&this.buffer.push(b)},render_pragmas:function(b){if(!this.includes("%",b))return b;
var u=this;return b.replace(RegExp(this.otag+"%([\\w-]+) ?([\\w]+=[\\w]+)?"+this.ctag),function(k,l,f){if(!u.pragmas_implemented[l])throw{message:"This implementation of mustache doesn't understand the '"+l+"' pragma"};u.pragmas[l]={};if(f){k=f.split("=");u.pragmas[l][k[0]]=k[1]}return""})},render_partial:function(b,u,k){b=this.trim(b);if(!k||k[b]===undefined)throw{message:"unknown_partial '"+b+"'"};if(typeof u[b]!="object")return this.render(k[b],u,k,true);return this.render(k[b],u[b],k,true)},render_section:function(b,
u,k){if(!this.includes("#",b)&&!this.includes("^",b))return b;var l=this;return b.replace(RegExp(this.otag+"(\\^|\\#)\\s*(.+)\\s*"+this.ctag+"\n*([\\s\\S]+?)"+this.otag+"\\/\\s*\\2\\s*"+this.ctag+"\\s*","mg"),function(f,m,p,r){f=l.find(p,u);if(m=="^")return!f||l.is_array(f)&&f.length===0?l.render(r,u,k,true):"";else if(m=="#")return l.is_array(f)?l.map(f,function(n){return l.render(r,l.create_context(n),k,true)}).join(""):l.is_object(f)?l.render(r,l.create_context(f),k,true):typeof f==="function"?
f.call(u,r,function(n){return l.render(n,u,k,true)}):f?l.render(r,u,k,true):""})},render_tags:function(b,u,k,l){var f=this,m=function(){return RegExp(f.otag+"(=|!|>|\\{|%)?([^\\/#\\^]+?)\\1?"+f.ctag+"+","g")},p=m(),r=function(I,B,L){switch(B){case "!":return"";case "=":f.set_delimiters(L);p=m();return"";case ">":return f.render_partial(L,u,k);case "{":return f.find(L,u);default:return f.escape(f.find(L,u))}};b=b.split("\n");for(var n=0;n<b.length;n++){b[n]=b[n].replace(p,r,this);l||this.send(b[n])}if(l)return b.join("\n")},
set_delimiters:function(b){b=b.split(" ");this.otag=this.escape_regex(b[0]);this.ctag=this.escape_regex(b[1])},escape_regex:function(b){if(!arguments.callee.sRE)arguments.callee.sRE=RegExp("(\\/|\\.|\\*|\\+|\\?|\\||\\(|\\)|\\[|\\]|\\{|\\}|\\\\)","g");return b.replace(arguments.callee.sRE,"\\$1")},find:function(b,u){b=this.trim(b);var k;if(u[b]===false||u[b]===0||u[b])k=u[b];else if(this.context[b]===false||this.context[b]===0||this.context[b])k=this.context[b];if(typeof k==="function")return k.apply(u);
if(k!==undefined)return k;return""},includes:function(b,u){return u.indexOf(this.otag+b)!=-1},escape:function(b){return String(b===null?"":b).replace(/&(?!\w+;)|["<>\\]/g,function(u){switch(u){case "&":return"&amp;";case "\\":return"\\\\";case '"':return'"';case "<":return"&lt;";case ">":return"&gt;";default:return u}})},create_context:function(b){if(this.is_object(b))return b;else{var u=".";if(this.pragmas["IMPLICIT-ITERATOR"])u=this.pragmas["IMPLICIT-ITERATOR"].iterator;var k={};k[u]=b;return k}},
is_object:function(b){return b&&typeof b=="object"},is_array:function(b){return Object.prototype.toString.call(b)==="[object Array]"},trim:function(b){return b.replace(/^\s*|\s*$/g,"")},map:function(b,u){if(typeof b.map=="function")return b.map(u);else{for(var k=[],l=b.length,f=0;f<l;f++)k.push(u(b[f]));return k}}};return{name:"mustache.js",version:"0.3.1-dev",to_html:function(b,u,k,l){var f=new h;if(l)f.send=l;f.render(b,u,k);if(!l)return f.buffer.join("\n")}}}();
(function(h){h.mustache=function(b,u,k){return Mustache.to_html(b,u,k)}})(jQuery);
...
Q+U).addClass(J+R+" "+J+Q+R).attr("title",s.togglerTip_open);da("all")}ea(c);m(c,!C.isSliding);v(c,s.onopen_end||s.onopen);if(ma)v(c,s.onshow_end||s.onshow);w(c)}var i=K[c],q=T[c],y=ca[c],s=n[c],C=B[c];e=!e&&C.isClosed&&s.fxName_open!="none";var x=A[c].edge,D=s.resizerClass,J=s.togglerClass,Q="-"+c,R="-open",U="-closed",Y="-sliding",ma=C.isShowing;delete C.isShowing;if(!(!i||!s.resizable&&!s.closable))if(C.isClosed||C.isSliding)if(C.isHidden&&!ma)ha(c,true);else if(A.isLayoutBusy)j("open",c,d);else if(false!==
...
i.stopPropagation()});p(e,d,!g.isClosed&&!g.isSliding);A[d].pins.push(c)}},allowOverflow:k,resetOverflow:l,cssWidth:G,cssHeight:M}}})(jQuery);
(function(){var h=function(){var b=null,u=function(k,l){var f=$($.mustache(l,k.tag));f.data("data",k);if(!b.tagMenu){b.tagMenu=f.find(".tag-menu");b.tagMenu.hide()}f.find(".tag-menu").remove();f.find(".color-selector").bind("click",function(m){var p=b.tagMenu;m=$(m.target);if(p.data("visible")===true&&p.data("tagId")===k.tag.id){p.hide("fast");p.data("visible",false);p.data("tagId",null);p.data("tag",null);p.data("target",null)}else{p.show("fast");p.css({left:f.position().left,top:f.position().top-
100});p.data("visible",true);p.data("tagId",k.tag.id);p.data("tag",k);p.data("target",m)}return false});ListKungFu&&ListKungFu.LayoutWest&&f.draggable({helper:"clone",appendTo:ListKungFu.LayoutWest,revert:"invalid",containment:ListKungFu.LayoutWest,start:function(){document.onselectstart=function(){return false};b._trigger("StartedDraggingTag",0,{dragType:"tag"})},stop:function(){document.onselectstart=null}});f.bind("click",function(){var m=f.data("data");if(f.data("selected")){m=m.tag.id;for(var p=
...
b.addTagButton=$('<button id="tag-new">Create Tag</button>');b.tagList=$('<div class="tags"></div>');b.element.append(b.toolbar);b.toolbar.append(b.addTagInput);b.toolbar.append(b.addTagButton);b.element.append(b.tagList);b.addTagButton.button({text:false,icons:{primary:"ui-icon-plusthick"}});b.selectedTags=[];Tag.Index({successCallback:function(k,l){for(var f=0;f<l.length;f++)b.tagList.append(u(l[f],k));b.tagList.append(b.tagMenu);b.tagMenu.find(".color").bind("click",function(m){var p=$(m.target).attr("data-colorclass");
...
$.trim(b.addTagInput.val())+"')").length===0){var k={};k.tag={name:b.addTagInput.val(),color_class:"c1"};Tag.Create({send:k,successCallback:function(l,f){var m=u(f,l);b.tagList.prepend(m);b.addTagInput.val("");b.addTagInput.trigger("keyup")}});return false}});b.addTagInput.bind("keyup","return",function(){b.addTagButton.trigger("click")});b.addTagInput.bind("keyup",function(){var k=$(this).val();if(k==="")b.tagList.find(".tag").show();else{b.tagList.find(".tag:not(:Contains('"+k+"'))").hide();b.tagList.find(".tag:Contains('"+
k+"')").show()}b.tagList.find(".tag-name:HasExactValue('"+$.trim(k)+"')").length>0?b.addTagButton.button("option","disabled",true):b.addTagButton.button("option","disabled",false)})},destroy:function(){b.element.children().remove()}}}();$.widget("ui.TagView",h)})();
